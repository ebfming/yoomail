...
namespace OCA\YooMail\Db;

use OCA\YooMail\Account;
use OCA\YooMail\Address;
use OCA\YooMail\AddressList;
use OCA\YooMail\Contracts\IMailSearch;
use OCA\YooMail\IMAP\Threading\DatabaseMessage;
use OCA\YooMail\Service\Search\Flag;
use OCA\YooMail\Service\Search\FlagExpression;
...
use OCA\YooMail\Support\PerformanceLoggerTask;
...
use function OCA\YooMail\array_flat_map;
use function strlen;
...
class MessageMapper extends QBMapper {
	...
	public function __construct(IDBConnection $db,
		...
		parent::__construct($db, 'yoomail_messages');
	...
	public function findByUserId(string $userId, int $id): Message {
		...
		$query->select('m.*')
			...
			->join('m', 'yoomail_mailboxes', 'mb', $query->expr()->eq('m.mailbox_id', 'mb.id', IQueryBuilder::PARAM_INT))
			->join('m', 'yoomail_accounts', 'a', $query->expr()->eq('mb.account_id', 'a.id', IQueryBuilder::PARAM_INT))
			->where(
	...
	public function findAccountIdForMessage(int $messageId): int {
		...
		$qb->select('mb.account_id')
			->from($this->getTableName(), 'm')
			->join('m', 'yoomail_mailboxes', 'mb', $qb->expr()->eq('m.mailbox_id', 'mb.id', IQueryBuilder::PARAM_INT))
	...
	public function deleteAll(Mailbox $mailbox): void {
		...
		$deleteRecipientsQuery->delete('yoomail_recipients')
	...
	public function deleteByUid(Mailbox $mailbox, int ...$uids): void {
		...
		$deleteMessagesQuery->delete('yoomail_messages')
			->where(
	...
	public function findThread(Account $account, string $threadRootId): array {
		...
		$qb->select('messages.*')
			...
			->join('messages', 'yoomail_mailboxes', 'mailboxes', $qb->expr()->eq('messages.mailbox_id', 'mailboxes.id', IQueryBuilder::PARAM_INT))
			->where(
	...
	public function findIdsByQuery(Mailbox $mailbox, SearchQuery $query, string $sortOrder, ?int $limit, ?array $uids = null): array {
		...
		if (!empty($query->getFrom())) {
			$select->innerJoin('m', 'yoomail_recipients', 'r0', 'm.id = r0.message_id');
		}
		if (!empty($query->getTo())) {
			$select->innerJoin('m', 'yoomail_recipients', 'r1', 'm.id = r1.message_id');
		}
		if (!empty($query->getCc())) {
			$select->innerJoin('m', 'yoomail_recipients', 'r2', 'm.id = r2.message_id');
		}
		if (!empty($query->getBcc())) {
			$select->innerJoin('m', 'yoomail_recipients', 'r3', 'm.id = r3.message_id');
		}
		...
		if (!empty($query->getTags())) {
			$select->innerJoin('m', 'yoomail_message_tags', 'tags', 'm.message_id = tags.imap_message_id');
			$select->andWhere(
	...
	public function findIdsGloballyByQuery(IUser $user, SearchQuery $query, ?int $limit, ?array $uids = null): array {
		...
		$selectMailboxIds = $qbMailboxes->select('mb.id')
			->from('yoomail_mailboxes', 'mb')
			->join('mb', 'yoomail_accounts', 'a', $qb->expr()->eq('a.id', 'mb.account_id', IQueryBuilder::PARAM_INT))
	...
	public function deleteOrphans(): void {
		...
		$idsQuery = $qb1->select('m.id')
			->from($this->getTableName(), 'm')
			->leftJoin('m', 'yoomail_mailboxes', 'mb', $qb1->expr()->eq('m.mailbox_id', 'mb.id'))
		...
		$recipientIdsQuery = $qb3->selectDistinct('r.id')
			->from('yoomail_recipients', 'r')
			->leftJoin('r', 'yoomail_messages', 'm', $qb3->expr()->eq('r.message_id', 'm.id'))
			->where(
	...
	public function findMessagesKnownSinceBefore(int $mailboxId, int $before): array {
		...
		$select = $qb->select('m.*')
			->from($this->getTableName(), 'm')
			->join('m', 'yoomail_messages_retention', 'mr', $qb->expr()->andX(
				$qb->expr()->eq(
					'm.mailbox_id',
					'mr.mailbox_id',
	...
	public function findMessagesToUnSnooze(int $mailboxId, int $time): array {
		...
		$select = $qb->select('m.*')
			->from($this->getTableName(), 'm')
			->join('m', 'yoomail_messages_snoozed', 'mr', $qb->expr()->andX(
				$qb->expr()->eq(
					'm.mailbox_id',
					'mr.mailbox_id',
					IQueryBuilder::PARAM_INT,
