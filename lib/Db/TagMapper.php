...
class TagMapper extends QBMapper {
	...
	public function __construct(IDBConnection $db,
		IL10N $l10n) {
		parent::__construct($db, 'yoomail_tags');
		$this->l10n = $l10n;
	}
	 ...
	 *
	 * To tag (flag) a message on IMAP, @see \OCA\YooMail\Service\MailManager::tagMessage
	 */
	public function tagMessage(Tag $tag, string $messageId, string $userId): void {
		...
		$qb = $this->db->getQueryBuilder();
		$qb->insert('yoomail_message_tags')
	...
	public function untagMessage(Tag $tag, string $messageId): void {
		$qb = $this->db->getQueryBuilder();
		$qb->delete('yoomail_message_tags')
	...
	public function getAllTagsForMessages(array $messages, string $userId): array {
		...
		$tagsQuery = $qb->selectDistinct(['t.*', 'mt.imap_message_id'])
			...
			->join('t', 'yoomail_message_tags', 'mt', $qb->expr()->eq('t.id', 'mt.tag_id', IQueryBuilder::PARAM_INT))
			->where(
	...
	public function deleteDuplicates(): void {
		$qb = $this->db->getQueryBuilder();
		$qb->select('mt2.id')
			->from('yoomail_message_tags', 'mt2')
			->join('mt2', 'yoomail_message_tags', 'mt1', $qb->expr()->andX(
		...
		$deleteQB->delete('yoomail_message_tags')
	...
	public function deleteOrphans(): void {
		...
		$qb->select('mt.id')
			->from('yoomail_messages', 'm')
			->rightJoin('m', 'yoomail_message_tags', 'mt', $qb->expr()->eq('m.message_id', 'mt.imap_message_id'))
		...
		$deleteMT->delete('yoomail_message_tags')
