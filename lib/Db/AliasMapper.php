...
class AliasMapper extends QBMapper {
	...
	public function __construct(IDBConnection $db) {
		parent::__construct($db, 'yoomail_aliases');
	}
	...
	public function find(int $aliasId, string $currentUserId): Alias {
		...
		$qb->select('aliases.*', 'accounts.provisioning_id')
			...
			->join('aliases', 'yoomail_accounts', 'accounts', $qb->expr()->eq('aliases.account_id', 'accounts.id'))
			->where(
				$qb->expr()->andX(
	...
	public function deleteProvisionedAliasesByUid(string $uid): void {
		...
		$idsQuery = $qb1->select('aliases.id')
			->from($this->getTableName(), 'aliases')
			->join('aliases', 'yoomail_accounts', 'accounts', $qb1->expr()->eq('aliases.account_id', 'accounts.id'))
			->where(
	...
	public function deleteOrphans(): void {
		...
		$idsQuery = $qb1->select('a.id')
			...
			->leftJoin('a', 'yoomail_accounts', 'ac', $qb1->expr()->eq('a.account_id', 'ac.id'))
