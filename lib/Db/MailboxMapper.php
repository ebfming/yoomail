...
class MailboxMapper extends QBMapper {
	...
	public function __construct(IDBConnection $db,
		...
		parent::__construct($db, 'yoomail_mailboxes');
	...
	public function findByUid(int $id, string $uid): Mailbox {
		...
		$select = $qb->select('mb.*')
			...
			->join('mb', 'yoomail_accounts', 'a', $qb->expr()->eq('mb.account_id', 'a.id', IQueryBuilder::PARAM_INT))
			->where(
	...
	public function deleteOrphans(): void {
		...
		$idsQuery = $qb1->select('m.id')
			...
			->leftJoin('m', 'yoomail_accounts', 'a', $qb1->expr()->eq('m.account_id', 'a.id'))
