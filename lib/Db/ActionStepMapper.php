...
class ActionStepMapper extends QBMapper {
	...
	public function __construct(IDBConnection $db) {
		parent::__construct($db, 'yoomail_action_step');
	}
	...
	public function find(int $id, string $owner): ActionStep {
		...
		$qb->select('step.*')
			...
			->join('step', 'yoomail_actions', 'actions', $qb->expr()->eq('step.action_id', 'actions.id'))
			->join('actions', 'yoomail_accounts', 'accounts', $qb->expr()->eq('actions.account_id', 'accounts.id'))
			->where(
				$qb->expr()->andX(
