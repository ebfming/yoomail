...
use OCA\YooMail\Http\JsonResponse as MailJsonResponse;
use OCA\YooMail\Http\TrapError;
use OCA\YooMail\Service\DelegationService;
use OCA\YooMail\Service\SieveService;
use OCA\YooMail\Sieve\SieveClientFactory;
...
class SieveController extends Controller {
	...
	public function updateActiveScript(int $id, string $script): JSONResponse {
		...
		try {
			...
			$this->logger->error('Installing sieve script failed: ' . $e->getMessage(), ['app' => 'yoomail', 'exception' => $e]);
