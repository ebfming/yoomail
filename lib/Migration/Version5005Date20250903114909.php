...
class Version5005Date20250903114909 extends SimpleMigrationStep {
	...
	public function changeSchema(IOutput $output, Closure $schemaClosure, array $options): ?ISchemaWrapper {
		...
		if ($schema->hasTable('yoomail_action_step')) {
			return null;
		}
		$table = $schema->createTable('yoomail_action_step');
		...
		if ($schema->hasTable('yoomail_actions')) {
			$table->addForeignKeyConstraint(
				$schema->getTable('yoomail_actions'),
				['action_id'],
				['id'],
				[
					'onDelete' => 'CASCADE',
		...
		if ($schema->hasTable('yoomail_mailboxes')) {
			$table->addForeignKeyConstraint(
				$schema->getTable('yoomail_mailboxes'),
				['mailbox_id'],
				['id'],
				[
					'onDelete' => 'SET NULL',
