...
class Version1130Date20220412111833 extends SimpleMigrationStep {
	...
	public function __construct(IDBConnection $connection, LoggerInterface $logger, ITempManager $tempManager) {
		...
		$this->backupPath = tempnam($tempBaseDir, 'yoomail_recipients_backup');
	}
	...
	public function preSchemaChange(IOutput $output, Closure $schemaClosure, array $options): void {
		...
		$sql1 = $this->connection->getDatabasePlatform()->getTruncateTableSQL('`*PREFIX*yoomail_recipients`', false);
		$this->connection->executeStatement($sql1);

		// Truncate and change primary key type for messages table
		$sql2 = $this->connection->getDatabasePlatform()->getTruncateTableSQL('`*PREFIX*yoomail_messages`', false);
		$this->connection->executeStatement($sql2);

		// Truncate message_tags table
		$sql3 = $this->connection->getDatabasePlatform()->getTruncateTableSQL('`*PREFIX*yoomail_message_tags`', false);
		$this->connection->executeStatement($sql3);

		// unset all locks for the mailboxes table
		$qb2 = $this->connection->getQueryBuilder();
		$qb2->update('yoomail_mailboxes')
	...
	public function changeSchema(IOutput $output, Closure $schemaClosure, array $options): ?ISchemaWrapper {
		...
		$attachmentsTable = $schema->getTable('yoomail_attachments');
		$fks = $attachmentsTable->getForeignKeys();
		foreach ($fks as $fk) {
			$attachmentsTable->removeForeignKey($fk->getName());
		}

		$recipientsTable = $schema->getTable('yoomail_recipients');
		...
		$recipientsTable->addForeignKeyConstraint($schema->getTable('yoomail_local_messages'), ['local_message_id'], ['id'], ['onDelete' => 'CASCADE'], 'yoomail_recip_local_msg');
		$attachmentsTable->addForeignKeyConstraint($schema->getTable('yoomail_local_messages'), ['local_message_id'], ['id'], ['onDelete' => 'CASCADE'], 'yoomail_attach_local_msg');

		$messagesTable = $schema->getTable('yoomail_messages');
		...
		// Add named indices
		$messagesTable->addIndex(['mailbox_id', 'flag_important', 'flag_deleted', 'flag_seen'], 'yoomail_messages_id_flags');
		$messagesTable->addIndex(['mailbox_id', 'flag_deleted', 'flag_flagged'], 'yoomail_messages_id_flags2');
		// Dropped in Version3600Date20240205180726 because mail_messages_mailbox_id is redundant with mail_messages_mb_id_uid
		// $messagesTable->addIndex(['mailbox_id'], 'yoomail_messages_mailbox_id');
		...
		if ($this->connection->getDatabasePlatform() instanceof PostgreSQLPlatform) {
			$messagesTable->addIndex(['mailbox_id', 'thread_root_id', 'sent_at'], 'yoomail_msg_thrd_root_snt_idx');
		} else {
			$messagesTable->addIndex(['mailbox_id', 'thread_root_id', 'sent_at'], 'yoomail_msg_thrd_root_snt_idx', [], ['lengths' => [null, 64, null]]);
		}
