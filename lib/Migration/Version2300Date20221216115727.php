...
class Version2300Date20221216115727 extends SimpleMigrationStep {
	...
	public function changeSchema(IOutput $output, Closure $schemaClosure, array $options): ?ISchemaWrapper {
		...
		$tableName = 'yoomail_smime_certificates';
		if (!$schema->hasTable($tableName)) {
			...
			$table->setPrimaryKey(['id'], 'yoomail_smime_certs_id_idx');
			$table->addIndex(['user_id'], 'yoomail_smime_certs_uid_idx');
			// Dropped in Version3600Date20240205180726
			// $table->addIndex(['id', 'user_id'], 'yoomail_smime_certs_id_uid_idx');
		}
