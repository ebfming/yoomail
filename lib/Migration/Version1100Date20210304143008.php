...
class Version1100Date20210304143008 extends SimpleMigrationStep {
	...
	public function changeSchema(IOutput $output, Closure $schemaClosure, array $options): ?ISchemaWrapper {
		...
		if (!$schema->hasTable('yoomail_tags')) {
			$tagsTable = $schema->createTable('yoomail_tags');
			...
			// $tagsTable->addIndex(['user_id'], 'yoomail_msg_tags_usr_id_index');
			$tagsTable->addUniqueIndex(
				[
					'user_id',
					'imap_label',
				],
				'yoomail_msg_tags_usr_lbl_idx'
			);
		}

		if (!$schema->hasTable('yoomail_message_tags')) {
			$tagsMessageTable = $schema->createTable('yoomail_message_tags');
