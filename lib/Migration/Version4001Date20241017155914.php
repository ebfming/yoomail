...
class Version4001Date20241017155914 extends SimpleMigrationStep {
	...
	public function changeSchema(IOutput $output, Closure $schemaClosure, array $options): ?ISchemaWrapper {
		...
		if ($schema->hasTable('yoomail_blocks_shares')) {
			return null;
		}
		$table = $schema->createTable('yoomail_blocks_shares');
		...
		$table->addUniqueIndex(['text_block_id', 'share_with'], 'yoomail_blk_shr_tbid_sw_idx');
		if ($schema->hasTable('yoomail_text_blocks')) {
			$table->addForeignKeyConstraint(
				$schema->getTable('yoomail_text_blocks'),
				['text_block_id'],
				['id'],
				[
					'onDelete' => 'CASCADE',
