...
use OCA\YooMail\Db\CollectedAddress;
...
class Version0110Date20180825201241 extends SimpleMigrationStep {
	...
	public function preSchemaChange(IOutput $output, \Closure $schemaClosure, array $options) {
		...
		if (!$schema->hasTable('yoomail_collected_addresses')) {
			return;
		}
		...
		// change table name
		$collectedAdressesMapper->setTableName('yoomail_collected_addresses');
		// change entity class
	...
	public function changeSchema(IOutput $output, \Closure $schemaClosure, array $options) {
		...
		if ($schema->hasTable('yoomail_collected_addresses')) {
			$schema->dropTable('yoomail_collected_addresses');
		}
