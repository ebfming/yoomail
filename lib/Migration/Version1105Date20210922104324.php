...
class Version1105Date20210922104324 extends SimpleMigrationStep {
	...
	public function postSchemaChange(IOutput $output, Closure $schemaClosure, array $options): void {
		...
		$qb->select('accounts.id')
			->from('yoomail_accounts', 'accounts')
			->leftJoin('accounts', 'yoomail_provisionings', 'provisionings', $qb->expr()->eq('accounts.provisioning_id', 'provisionings.id'))
