...
class Version1020Date20191002091035 extends SimpleMigrationStep {
	...
	public function changeSchema(IOutput $output, Closure $schemaClosure, array $options) {
		...
		$messagesTable = $schema->createTable('yoomail_messages');
		...
		$messagesTable->addIndex(['sent_at'], 'yoomail_message_sent_idx');

		$recipientsTable = $schema->createTable('yoomail_recipients');
		...
		$recipientsTable->addIndex(['message_id'], 'yoomail_recipient_msg_id_idx');

		// Converted to wider index later on - installations might have the old one until replacement
		// $recipientsTable->addIndex(['email'], 'yoomail_recipient_email_idx');
		$recipientsTable->addIndex(['email', 'type', 'message_id'], 'yoomail_recip_eml_type_mid_idx');
