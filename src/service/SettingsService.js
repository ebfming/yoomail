...
export function getProvisioningSettings(config) {
	const url = generateUrl('/apps/yoomail/api/settings/provisioning')

	return axios.get(url, config).then((resp) => resp.data)
}

export function provisionAll() {
	const url = generateUrl('/apps/yoomail/api/settings/provisioning/all')
...
export function updateProvisioningSettings(config) {
	const url = generateUrl('/apps/yoomail/api/settings/provisioning/{id}', {
		id: config.id,
	})
	const data = {
		data: config,
...
export function setAntiSpamEmail(email) {
	return axios.post(generateUrl('/apps/yoomail/api/settings/antispam'), { spam: email.spam, ham: email.ham })
		.then((resp) => resp.data)
}

export function deleteAntiSpamEmail() {
	return axios.delete(generateUrl('/apps/yoomail/api/settings/antispam'))
		.then((resp) => resp.data)
}

export function updateAllowNewMailAccounts(allowed) {
	const url = generateUrl('/apps/yoomail/api/settings/allownewaccounts')
	const data = {
		allowed,
	}
	return axios.post(url, data).then((resp) => resp.data)
...
export async function updateLlmEnabled(enabled) {
	const url = generateUrl('/apps/yoomail/api/settings/llm')
	const data = {
		enabled,
	}
...
export async function updateEnabledSmartReply(enabled) {
	const url = generateUrl('/apps/yoomail/api/settings/smartreply')
	const data = {
		enabled,
	}
	const resp = await axios.put(url, data)
...
export async function setLayoutMessageView(value) {
	const url = generateUrl('/apps/yoomail/api/settings/layout-message-view')
	const data = {
		value,
	}
	const resp = await axios.put(url, data)
...
export async function setImportanceClassificationEnabledByDefault(enabledByDefault) {
	const url = generateUrl('/apps/yoomail/api/settings/importance-classification-default')
	await axios.put(url, {
		enabledByDefault,
	})
}
