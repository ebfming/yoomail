		...
		<AppContent>
			<div
				...
				<EmptyContent
					...
					:name="t('yoomail', 'Connect your mail account')">
					<template #icon>
				...
				<EmptyContent v-else :name="t('yoomail', 'To add a mail account, please contact your administrator.')">
					<template #icon>
						<div class="setup__form-content__svg-wrapper" v-html="FluidMail" />
					</template>
				</EmptyContent>
...
export default {
	...
	data() {
		return {
			displayName: loadState('yoomail', 'prefill_displayName'),
			email: loadState('yoomail', 'prefill_email'),
			FluidMail,
			allowNewMailAccounts: loadState('yoomail', 'allow-new-accounts', true),
			error: null,
			backgroundImgSrc: this.isDarkTheme
				? 'url("' + generateFilePath('yoomail', 'img', 'welcome-connection-dark.png') + '")'
				: 'url("' + generateFilePath('yoomail', 'img', 'welcome-connection-light.png') + '")',
