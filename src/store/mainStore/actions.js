	...
	return {
		...
		async startComposerSession({
			...
			if (this.composerSessionId && !this.composerMessageIsSaved) {
				...
				const discard = await new Promise((resolve) => OC.dialogs.confirmDestructive(
					t('yoomail', 'There is already a message in progress. All unsaved changes will be lost if you continue!'),
					t('yoomail', 'Discard changes'),
					{
						type: OC.dialogs.YES_NO_BUTTONS,
						confirm: t('yoomail', 'Discard unsaved changes'),
						confirmClasses: 'error',
						cancel: t('yoomail', 'Keep editing message'),
					},
					(decision) => {
						resolve(decision)
					},
			...
			return handleHttpAuthErrors(async () => {
				if (reply) {
					...
					if (reply?.smartReply) {
						const aiDisclaimerText = t('yoomail', '(All or part of this reply was generated by AI)')
					...
					if (message.attachments.length) {
						showWarning(t('yoomail', 'Attachments were not copied. Please add them manually.'))
					}
				}
		...
		async deleteMessage({ id }) {
			return handleHttpAuthErrors(async () => {
				...
				try {
					...
					return {
						remoteMissing: response?.data?.remoteMissing === true,
		...
		async deleteThread({ envelope }) {
			return handleHttpAuthErrors(async () => {
				...
				try {
					const response = await ThreadService.deleteThread(envelope.databaseId)
		...
		async createAndSetSnoozeMailbox(account) {
			...
			if (snoozeMailboxId === undefined) {
				...
				showError(t('yoomail', 'Could not create snooze mailbox'))
				return
			}
		...
		addEnvelopeThreadMutation({
			...
			thread.forEach((e) => {
				...
				// The mailbox may not be loaded yet (e.g. when opening a thread
				// URL directly). Fall back to the existing envelope's accountId,
				// otherwise leave it unset rather than crashing the whole fetch.
				if (mailbox !== undefined) {
					Vue.set(e, 'accountId', mailbox.accountId)
				} else if (this.envelopes[e.databaseId]?.accountId !== undefined) {
					Vue.set(e, 'accountId', this.envelopes[e.databaseId].accountId)
				}
		...
		async processHtmlBody(id) {
			try {
				...
				if (error.response?.status === 404) {
					showError(t('yoomail', 'Sorry, the message could not be loaded. The draft may no longer exist. Please refresh the page and try again.'), { timeout: TOAST_DEFAULT_TIMEOUT * 2 })
				}
				throw error
			}
		},
