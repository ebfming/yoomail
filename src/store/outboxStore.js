	...
	actions: {
			...
			return new Promise((resolve, reject) => {
				...
				setTimeout(async () => {
					try {
						...
						if (wasSent) {
							showSuccess(t('yoomail', 'Message sent'))
						}
						resolve(wasSent)
					} catch (error) {
						showError(t('yoomail', 'Could not send message'))
		...
		async copyMessageToSentMailbox({ id }) {
			...
			try {
				...
				showSuccess(t('yoomail', 'Message copied to "Sent" folder'))
			} catch (error) {
				showError(t('yoomail', 'Could not copy message to "Sent" folder'))
