...
export function getRandomFolderErrorMessage(folder) {
	...
	const rawTexts = [
		t('yoomail', 'Could not load {tag}{name}{endtag}', {
			name: folderName,
		}),
		t('yoomail', 'Could not load {tag}{name}{endtag}', {
			name: folderName,
		}),
		t('yoomail', 'There was a problem loading {tag}{name}{endtag}', {
			name: folderName,
		}),
	]
...
export function getRandomMessageErrorMessage() {
	const texts = [
		t('yoomail', 'Could not load your message'),
		t('yoomail', 'Could not load the desired message'),
		t('yoomail', 'Could not load the message'),
	]
	const text = texts[Math.floor(Math.random() * texts.length)]
	return text + ' ' + getRandomSmiley()
}
