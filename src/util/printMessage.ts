...
export function buildMessageHeader(doc: Document, envelope: PrintEnvelope): HTMLElement {
	...
	subject.textContent = envelope.subject?.trim() ? envelope.subject : t('yoomail', 'No subject')
	...
	addLine(t('yoomail', 'From:'), formatRecipients(envelope.from))
	addLine(t('yoomail', 'To:'), formatRecipients(envelope.to))
	addLine(t('yoomail', 'Cc:'), formatRecipients(envelope.cc))
	addLine(t('yoomail', 'Bcc:'), formatRecipients(envelope.bcc))
	addLine(t('yoomail', 'Date:'), envelope.dateInt ? formatDateTimeFromUnix(envelope.dateInt) : '')
...
function browserPrintTitle(): string {
	return t('yoomail', 'Printing from the browser menu is not supported')
}
...
function browserPrintHint(): string {
	return t('yoomail', 'To print this email, press {shortcut}, or use the "Print message" action in the message menu.', {
