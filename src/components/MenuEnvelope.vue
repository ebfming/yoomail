	...
	<div>
		<template v-if="!localMoreActionsOpen && !snoozeActionsOpen">
			...
			<ActionButton
				...
				<template #icon>
					<AlertOctagonIcon
						:title="envelope.flags.$junk ? t('yoomail', 'Mark not spam') : t('yoomail', 'Mark as spam')"
						:size="20" />
				</template>
				{{
					envelope.flags.$junk ? t('yoomail', 'Mark not spam') : t('yoomail', 'Mark as spam')
				}}
			</ActionButton>
			<ActionButton
				v-if="hasWriteAcl"
		...
		<template v-if="localMoreActionsOpen">
			...
			<ActionButton
				...
				<template #icon>
					<ShareIcon
						:title="t('yoomail', 'Forward message as attachment')"
						:size="20" />
				</template>
				{{ t('yoomail', 'Forward message as attachment') }}
			</ActionButton>
			<ActionButton
				:close-after-click="true"
				...
				<template #icon>
					<PlusIcon
						:title="t('yoomail', 'Edit as new message')"
						:size="20" />
				</template>
			...
			<ActionButton
				...
				<template #icon>
					<CalendarBlankIcon
						:title="t('yoomail', 'Reply with meeting')"
						:size="20" />
				</template>
			...
			<ActionButton
				...
				<template #icon>
					<InformationIcon
						:title="t('yoomail', 'View source')"
						:size="20" />
				</template>
				{{ t('yoomail', 'View source') }}
			</ActionButton>
			<ActionButton
				:close-after-click="true"
				@click="onPrint">
			...
			<ActionButton
				...
				<template #icon>
					<FilterIcon
						:title="t('yoomail', 'Create mail filter')"
						:size="20" />
				</template>
				{{ t('yoomail', 'Create mail filter') }}
			</ActionButton>
			<ActionLink
				v-if="debug"
				...
				<template #icon>
					<DownloadIcon
						:title="t('yoomail', 'Download thread data for debugging')"
						:size="20" />
				</template>
				{{ t('yoomail', 'Download thread data for debugging') }}
			</ActionLink>
		</template>
	...
	methods: {
		...
		async onSnooze(timestamp) {
			...
			try {
				...
				showSuccess(t('yoomail', 'Message was snoozed'))
			} catch (error) {
				logger.error('Could not snooze message', error)
				showError(t('yoomail', 'Could not snooze message'))
			}
		},
		...
		async onUnSnooze() {
			...
			try {
				...
				showSuccess(t('yoomail', 'Message was unsnoozed'))
			} catch (error) {
				logger.error('Could not unsnooze message', error)
				showError(t('yoomail', 'Could not unsnooze message'))
			}
		},
