...
<template>
	<AppContent
				...
				<AppContentList
					...
					<template v-if="!mailbox.isPriorityInbox">
						<div
							...
							<SectionTitle
								class="section-title"
								:name="t('yoomail', 'Favorites')" />
							<NcPopover trigger="hover focus">
								<template #trigger>
									<ButtonVue
										type="tertiary-no-background"
										:aria-label="t('yoomail', 'Favorites info')"
										class="button">
										<template #icon>
											<IconInfo :size="20" />
										</template>
						...
						<Mailbox
							v-if="sortFavorites"
							v-show="hasFavoriteEnvelopes"
							:load-more-label="t('yoomail', 'Load more favorites')"
							:account="account"
							:mailbox="mailbox"
					...
					<template v-else>
						...
						<div
							...
							<SectionTitle
								class="section-title"
								:name="t('yoomail', 'Follow up')" />
							<NcPopover trigger="hover focus">
								<template #trigger>
									<ButtonVue
										type="tertiary-no-background"
										:aria-label="t('yoomail', 'Follow up info')"
										class="button">
										<template #icon>
											<IconInfo :size="20" />
										</template>
						...
						<Mailbox
							v-show="hasFollowUpEnvelopes"
							:load-more-label="t('yoomail', 'Load more follow ups')"
							:account="unifiedAccount"
						...
						<div v-show="hasImportantEnvelopes" class="app-content-list-item">
							<SectionTitle
								...
								:name="t('yoomail', 'Important')" />
							<NcPopover trigger="hover focus">
								<template #trigger>
									<ButtonVue
										type="tertiary-no-background"
										:aria-label="t('yoomail', 'Important info')"
										class="button">
										<template #icon>
											<IconInfo :size="20" />
										</template>
						...
						<Mailbox
							...
							class="nameimportant"
							:load-more-label="t('yoomail', 'Load more important messages')"
							:account="unifiedAccount"
							:mailbox="unifiedInbox"
		...
		<Thread v-if="showThread" @delete="deleteMessage" @thread-not-found="hideMessage" />
...
export default {
	...
	data() {
		return {
			...
			importantInfo: t('yoomail', 'Messages will automatically be marked as important using AI. The system learns from which messages you interact with or mark as important. In the beginning you might have to manually change the importance to teach it, but it will improve over time'),
			favoritesInfo: t('yoomail', 'Messages that you marked as favorite will be shown at the top of folders. You can disable this behavior in the app settings'),
			followupInfo: t('yoomail', 'AI identifies messages sent by you that likely require a reply but did not receive one after a couple of days and shows them here'),
			bus: mitt(),
