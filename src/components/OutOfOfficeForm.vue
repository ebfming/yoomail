	...
	<form class="form" @submit.prevent="submit">
		<div class="form__multi-row">
			<fieldset class="form__fieldset">
				...
				<label for="ooo-disabled">{{ t('yoomail', 'Autoresponder off') }}</label>
			</fieldset>
			...
			<fieldset class="form__fieldset">
				...
				<label for="ooo-enabled">{{ t('yoomail', 'Autoresponder on') }}</label>
			</fieldset>
			...
			<fieldset v-if="hasPersonalAbsenceSettings" class="form__fieldset">
				...
				<label for="ooo-follow-system">{{ t('yoomail', 'Autoresponder follows system settings') }}</label>
			</fieldset>
		</div>

		<template v-if="followingSystem">
			<p>{{ t('yoomail', 'The autoresponder follows your personal absence period settings.') }}</p>
			<ButtonVue :href="personalAbsenceSettingsUrl" target="_blank" rel="noopener noreferrer">
				<template #icon>
					<OpenInNewIcon :size="20" />
				</template>
				{{ t('yoomail', 'Edit absence settings') }}
			</ButtonVue>
		</template>
		<template v-else>
			<div class="form__multi-row">
				<fieldset class="form__fieldset">
					<label for="ooo-first-day">{{ t('yoomail', 'First day') }}</label>
					<DatetimePicker
						id="ooo-first-day"
						v-model="firstDay"
						:disabled="!enabled" />
			...
			<fieldset class="form__fieldset">
				<label for="ooo-subject">{{ t('yoomail', 'Subject') }}</label>
				<input
					id="ooo-subject"
					v-model="subject"
					type="text"
				...
				<p class="form__fieldset__description">
					{{ t('yoomail', '${subject} will be replaced with the subject of the message you are responding to') }}
				</p>
			</fieldset>

			<fieldset class="form__fieldset">
				<label for="ooo-message">{{ t('yoomail', 'Message') }}</label>
				<TextEditor
					id="ooo-message"
					v-model="message"
					:html="false"
		...
		<ButtonVue
			type="primary"
			native-type="submit"
			:aria-label="t('yoomail', 'Save autoresponder')"
			:disabled="loading || !valid">
			<template #icon>
				<CheckIcon :size="20" />
			</template>
			{{ t('yoomail', 'Save autoresponder') }}
		</ButtonVue>
	</form>
</template>
...
export default {
	...
	data() {
		...
		const enableSystemOutOfOffice = loadState('yoomail', 'enable-system-out-of-office', false)
