...
<template>
	<Modal @close="onClose">
		<div class="modal-content">
			<h2>{{ t('yoomail', 'Create task') }}</h2>
			<div class="taskTitle">
				<label for="taskTitle">{{ t('yoomail', 'Title') }}</label>
			...
			<label for="note">{{ t('yoomail', 'Description') }}</label>
			...
			<NcSelect
				...
				label="displayname"
				input-id="url"
				:placeholder="t('yoomail', 'Select calendar')"
				...
				<template #no-options>
					<span>{{ t('yoomail', 'No calendars with task list support') }}</span>
				</template>
			</NcSelect>
			<br>
	...
	methods: {
		...
		async onSave() {
			...
			try {
				...
				showSuccess(t('yoomail', 'Task created'))

				this.onClose()
			} catch (error) {
				showError(t('yoomail', 'Could not create task'))
