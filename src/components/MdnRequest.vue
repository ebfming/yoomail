...
<template>
	<div v-if="hasMdnRequest && !mdnSent" class="mail-message-has-mdn-request">
		{{
			t('yoomail', 'The sender of this message has asked to be notified when you read this message.')
		}}
		<div class="notify-button">
			<NcButton variant="secondary" :disabled="loading" @click="sendMdn">
				{{ t('yoomail', 'Notify the sender') }}
			</NcButton>
		</div>
	</div>
	<div v-else-if="mdnSent" class="mail-message-has-mdn-request">
		{{
			t('yoomail', 'You sent a read confirmation to the sender of this message.')
		}}
	</div>
</template>
	...
	methods: {
		async sendMdn() {
			...
			try {
				...
				showError(t('yoomail', 'Could not send mdn'))
			}
