	...
	<ListItem
		...
		<template #actions>
			<ActionButton
				...
				{{ t('yoomail', 'Copy to "Sent" Folder') }}
				<template #icon>
					<Copy
						:title="t('yoomail', 'Copy to Sent Folder')"
						:size="20" />
				</template>
			</ActionButton>
			<ActionButton
	...
	computed: {
		...
		details() {
			if (this.message.status === STATUS_IMAP_SENT_MAILBOX_FAIL) {
				return this.t('yoomail', 'Could not copy to "Sent" folder')
			} else if (this.message.status === STATUS_SMTP_ERROR) {
				return this.t('yoomail', 'Mail server error')
			} else if (this.message.status !== STATUS_RAW) {
				return this.t('yoomail', 'Message could not be sent')
			}
		...
		subjectForSubtitle() {
			...
			// or undefined.
			return this.message.subject || this.t('yoomail', 'No subject')
		},
	},
	...
	methods: {
		...
		async deleteMessage() {
			try {
				...
				showSuccess(t('yoomail', 'Message deleted'))
			} catch (error) {
