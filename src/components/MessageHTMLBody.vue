...
<template>
	<div class="html-message-body">
		...
		<div v-if="hasBlockedContent" id="mail-message-has-blocked-content" style="color: #000000">
			{{ t('yoomail', 'The images have been blocked to protect your privacy.') }}
			<Actions type="tertiary" :menu-name="t('yoomail', 'Show images')">
				<ActionButton @click="displayIframe">
					<template #icon>
						<IconImage :size="20" />
					</template>
					{{ t('yoomail', 'Show images temporarily') }}
				</ActionButton>
				<ActionButton
					v-if="sender"
					...
					{{ t('yoomail', 'Always show images from {sender}', { sender }) }}
				</ActionButton>
				<ActionButton
					v-if="domain"
					...
					{{ t('yoomail', 'Always show images from {domain}', { domain }) }}
				</ActionButton>
			</Actions>
		</div>
