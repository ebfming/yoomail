	...
	<div>
		<NcListItem
			...
			<template #extra-actions>
				<NcActionButton
					:title="t('yoomail', 'Remove')"
					:aria-label="t('yoomail', 'Remove')"
		...
		<ButtonVue
			...
			{{ t('yoomail', 'Add internal address') }}
		</ButtonVue>
		<NcDialog
			:open.sync="openDialog"
			:buttons="buttons"
			:name="t('yoomail', 'Add internal address')"
			@close="openDialog = false">
			<NcTextField v-model="newAddress" class="input" :label="t('yoomail', 'Add internal email or domain')" />
		</NcDialog>
	</div>
</template>
	...
	methods: {
		async removeInternalAddress(sender) {
			...
			try {
				...
				showError(t('yoomail', 'Could not remove internal address {sender}', {
		...
		async addInternalAddress() {
			...
			try {
				...
				showError(t('yoomail', 'Could not add internal address {address}', {
					address: this.newAddress,
				}))
			}
		},
		...
		senderType(type) {
			switch (type) {
				case 'individual':
					return t('yoomail', 'individual')
				case 'domain':
					return t('yoomail', 'domain')
			}
			return type
		},
	},
