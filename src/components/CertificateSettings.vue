...
<template>
	<div class="certificate-settings">
		<NcSelect
			...
			:options="aliases"
			:searchable="false"
			:placeholder="t('yoomail', 'Select an alias')"
			:aria-label-combobox="t('yoomail', 'Select an alias')"
			label="name"
		...
		<NcSelect
			...
			:aria-label-combobox="t('yoomail', 'Select certificates')"
			:searchable="false" />
		<NcButton
			variant="primary"
			class="certificate-settings__submit"
			:disabled="certificate === null"
			:aria-label="t('yoomail', 'Update Certificate')"
			@click="updateSmimeCertificate">
			{{ t('yoomail', 'Update Certificate') }}
		</NcButton>
		<NcNoteCard
			v-if="alias && !savedCertificate.isChainVerified"
			type="warning">
			<p>{{ t('yoomail', 'The selected certificate is not trusted by the server. Recipients might not be able to verify your signature.') }}</p>
		</NcNoteCard>
	</div>
</template>
...
export default {
	...
	computed: {
		...
		noCertificateOption() {
			return {
				label: this.t('yoomail', 'No certificate'),
	...
	methods: {
		async updateSmimeCertificate() {
			if (this.alias.isAccountCertificate) {
				await this.mainStore.updateAccountSmimeCertificate({
					...
					showSuccess(t('yoomail', 'Certificate updated'))
				}).catch((error) => {
					Logger.error('could not update account Smime ceritificate', { error })
					showError(t('yoomail', 'Could not update certificate'))
				})
			} else {
		...
		mapCertificateToOption(cert) {
			const label = this.t('yoomail', '{commonName} - Valid until {expiryDate}', {
