...
<template>
	<div v-if="attachments.length > 0" class="mail-message-attachments" :class="hasNextLine ? 'has-next-line' : ''">
		...
		<NcButton
			...
			<span v-if="isToggled">
				{{ n('yoomail', 'View {count} more attachment', 'View {count} more attachments', (attachments.length - visible), { count: attachments.length - visible }) }}
			</span>
			<span v-else>
				{{ t('yoomail', 'View fewer attachments') }}
			</span>
		</NcButton>
		...
		<p v-if="moreThanOne" class="attachments-button-wrapper">
			<FilePicker
				...
				:name="t('yoomail', 'Choose a folder to store the attachments in')"
...
export default {
	...
	computed: {
		...
		zipUrl() {
			return generateUrl('/apps/yoomail/api/messages/{id}/attachments', {
	...
	methods: {
		...
		saveAll(dest) {
			...
			saveAttachmentsToFiles(id, path).then(() => {
				Logger.info('saved')
				showSuccess(t('yoomail', 'Attachments saved to Files'))
			}).catch((error) => {
				Logger.error('not saved', error)
				showError(t('yoomail', 'Error while saving attachments'))
			}).finally(() => {
