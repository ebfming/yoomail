		...
		<NcAppNavigationCaption
			...
			<template #actions>
				<template v-if="isDisabled">
					<ActionText :name="t('yoomail', 'Provisioned account is disabled')">
						<template #icon>
							<IconInfo :size="20" />
						</template>
						{{ t('yoomail', 'Please login using a password to enable this account. The current session is using passwordless authentication, e.g. SSO or WebAuthn.') }}
					</ActionText>
				</template>
				<template v-else>
...
export default {
	...
	computed: {
		...
		quotaText() {
			if (this.quota) {
				return t('yoomail', 'Used quota: {quota}% ({limit})', {
			...
			if (this.account.quotaPercentage) {
				return t('yoomail', 'Used quota: {quota}%', {
	...
	methods: {
		...
		async removeAccount() {
			...
			const dialog = new DialogBuilder()
				.setName(t('yoomail', 'Remove account'))
				.setText(t('yoomail', 'The account for {email} and cached email data will be removed from Nextcloud, but not from your email provider.', { email: this.account.emailAddress }))
				.setButtons([
					{
						label: t('yoomail', 'Cancel'),
					},
					{
						label: t('yoomail', 'Remove {email}', { email: this.account.emailAddress }),
						type: 'error',
						callback: async () => {
							...
							try {
								...
								location.href = generateUrl('/apps/yoomail')
							} catch (error) {
								logger.error('could not delete account', { error })
								showError(t('yoomail', 'could not delete account'))
							} finally {
