		...
		<template>
			<div class="contact-wrapper">
				...
				<ButtonVue
					...
					:aria-label="t('yoomail', 'Contacts with this address')"
					class="contact-existing">
					<template #icon>
						<IconDetails :size="20" />
					</template>
					{{ t('yoomail', 'Contacts with this address') }}: {{ contactsWithEmailComputed }}
				</ButtonVue>
				<div v-if="selection === ContactSelectionStateEnum.select" class="contact-menu">
					<ButtonVue
						:aria-label="t('yoomail', 'Reply')"
						type="tertiary-no-background"
						@click="onClickReply">
						<template #icon>
							<IconReply :size="20" />
					...
					<ButtonVue
						type="tertiary-no-background"
						:aria-label="t('yoomail', 'Add to Contact')"
						@click="selection = ContactSelectionStateEnum.existing">
						<template #icon>
							<IconUser :size="20" />
						</template>
						{{ t('yoomail', 'Add to Contact') }}
					</ButtonVue>
					<ButtonVue
						type="tertiary-no-background"
						:aria-label="t('yoomail', 'New Contact')"
						@click="selection = ContactSelectionStateEnum.new">
						<template #icon>
							<IconAdd :size="20" />
						</template>
						{{ t('yoomail', 'New Contact') }}
					</ButtonVue>
					<ButtonVue
						type="tertiary-no-background"
						:aria-label="t('yoomail', 'Copy to clipboard')"
						@click="onClickCopyToClipboard">
						<template #icon>
							<IconClipboard :size="20" />
						</template>
						{{ t('yoomail', 'Copy to clipboard') }}
					</ButtonVue>
				</div>
	...
	methods: {
		async onClickCopyToClipboard() {
			try {
				...
				showSuccess(t('yoomail', 'Copied email address to clipboard'))
			} catch (e) {
				logger.error('could not copy email address to clipboard', { error: e })
				showError(t('yoomail', 'Could not copy email address to clipboard'))
			}
		},
