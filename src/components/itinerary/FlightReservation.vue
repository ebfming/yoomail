...
<template>
	<div class="reservation">
		...
		<div class="connection">
			<div><AirplaneIcon :title="t('yoomail', 'Airplane')" /></div>
			<div>{{ flightNumber }}</div>
			<div v-if="reservation">
				{{ t('yoomail', 'Reservation {id}', { id: reservation }) }}
			</div>
			<div v-else>
	...
	methods: {
		handleImport(calendar) {
			...
			event.updatePropertyWithValue(
				'SUMMARY',
				t('yoomail', 'Flight {flightNr} from {depAirport} to {arrAirport}', {
			...
			return importCalendarEvent(calendar.url)(cal.toString())
				.then(() => {
					...
					showSuccess(t('yoomail', 'Event imported into {calendar}', { calendar: calendar.displayname }))
				})
				.catch((error) => {
