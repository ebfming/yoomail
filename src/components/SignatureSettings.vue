	...
	<div class="section">
		<div>
			...
			<label for="signature-above-quote-toggle">
				{{ t("yoomail", "Place signature above quoted text") }}
			</label>
		</div>
		<NcSelect
		...
		<div class="signature-editor-wrapper">
			...
			<TextEditor
				...
				v-model="signature"
				:html="allowHtml"
				:placeholder="t('yoomail', 'Signature …')"
				:bus="bus"
		...
		<NcNoteCard v-if="isLargeSignature" type="warning">
			...
			<p>{{ t('yoomail', 'This signature is larger than 2 MB, usually because an image is embedded in it. It is added to every message you send and may slow down the editor.') }}</p>
		</NcNoteCard>
		<NcNoteCard v-if="overridesPlainText" type="warning">
			...
			<p>{{ t('yoomail', 'This signature contains images. New messages will use rich text, even though your writing mode is set to plain text.') }}</p>
		</NcNoteCard>
		<ButtonVue
			type="primary"
			:disabled="loading"
			:aria-label="t('yoomail', 'Save signature')"
			@click="saveSignature">
			<template #icon>
				<IconLoading v-if="loading" :size="20" fill-color="white" />
				<IconCheck v-else :size="20" />
			</template>
			{{ t('yoomail', 'Save signature') }}
		</ButtonVue>
		<ButtonVue
			v-if="signature"
			:aria-label="t('yoomail', 'Delete')"
