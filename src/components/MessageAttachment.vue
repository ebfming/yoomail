...
<template>
	<div class="attachment" :class="{ 'message-attachment--can-preview': canPreview }">
		...
		<FilePicker
			...
			:name="t('yoomail', 'Choose a folder to store the attachment in')"
		...
		<Actions :boundaries-element="boundariesElement">
			<template v-if="!showCalendarPopover">
				<ActionButton
					...
					{{ t('yoomail', 'Import into calendar') }}
				</ActionButton>
				<ActionButton
	...
	computed: {
		name() {
			if (this.mime === 'message/rfc822') {
				return t('yoomail', 'Embedded message')
			}
		...
		label() {
			if (this.mime === 'message/rfc822') {
				return t('yoomail', 'Embedded message') + ' (' + formatFileSize(this.size) + ')'
			}
	...
	methods: {
		...
		async saveToCloud(dest) {
			...
			try {
				...
				Logger.info('saved')
				showSuccess(t('yoomail', 'Attachment saved to Files'))
			} catch (e) {
				Logger.error('not saved', { error: e })
				showError(t('yoomail', 'Attachment could not be saved'))
			} finally {
		...
		importCalendar(url) {
			return () => {
				downloadAttachment(this.url)
					...
					.then(() => {
						showSuccess(t('yoomail', 'calendar imported'))
					})
					.catch((error) => {
