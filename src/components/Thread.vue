...
<template>
	<AppContentDetails id="mail-message">
		...
		<Loading v-if="loading && thread.length === 0" :hint="t('yoomail', 'Loading thread')" />
		<Error
			v-else-if="errorTitle || errorMessage"
			:error="errorTitle ? errorTitle : t('yoomail', 'Not found')"
			:message="errorMessage">
			<template v-if="errorRetry" #action>
				<NcButton
					:aria-label="t('yoomail', 'Refresh mailbox')"
					@click="onRetry">
					{{ t('yoomail', 'Refresh and retry') }}
				</NcButton>
			</template>
		</Error>
		<template v-else>
...
import { NcAppContentDetails as AppContentDetails, NcButton } from '@nextcloud/vue'
...
export default {
	...
	data() {
		return {
			...
			errorRetry: false,
			expandedThreads: [],
			enabledThreadSummary: loadState('yoomail', 'llm_summaries_available', false),
			summaryText: '',
			summaryError: false,
			isolatedPrint: false,
		}
	...
	computed: {
		...
		threadSubject() {
			...
			return thread[0].subject || this.t('yoomail', 'No subject')
		},
	...
	methods: {
		async updateSummary() {
			...
			try {
				...
				showError(t('yoomail', 'Summarizing thread failed.'))
		...
		async fetchThread() {
			...
			try {
				...
				if (error?.response?.status === 403) {
					...
					this.errorMessage = t('yoomail', 'Could not load your message thread')
					this.errorRetry = true
					this.loading = false
				}
			}
		...
		/**
		 * Retry loading the thread after a "deleted/not found" error. Since the
		 * email is likely gone, we refresh the mailbox and navigate back to the
		 * list so the stale message pane does not linger.
		 */
		async onRetry() {
			this.errorMessage = ''
			this.errorTitle = ''
		...
		async printMessages(indices) {
			...
			try {
				...
				showError(t('yoomail', 'Could not print message'))
			}
		},
