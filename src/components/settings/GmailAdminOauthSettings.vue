...
<template>
	<form @submit.prevent="onSubmit">
		<label for="mail-gmail-oauth-client-id"> {{ t('yoomail', 'Client ID') }} </label>
		<input
			...
			type="text"
			required>
		<label for="mail-gmail-oauth-client-secret"> {{ t('yoomail', 'Client secret') }} </label>
		<input
	...
	methods: {
		async onSubmit() {
			...
			try {
				...
				showSuccess(t('yoomail', 'Google integration configured'))
			} catch (error) {
				logger.error('Could not configure Google integration', { error })
				showError(t('yoomail', 'Could not configure Google integration'))
			} finally {
				this.loading = false
			}
		},
		...
		async onUnlink() {
			...
			try {
				...
				this.clientIdVal = ''
				this.clientSecret = ''
				showSuccess(t('yoomail', 'Google integration unlinked'))
			} catch (error) {
				logger.error('Could not unlink Google integration', { error })
				showError(t('yoomail', 'Could not unlink Google integration'))
			} finally {
				this.loading = false
			}
		},
