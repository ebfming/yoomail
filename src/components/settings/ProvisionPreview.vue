...
<template>
	<div class="provision-preview">
		...
		<br>
		{{ t('yoomail', 'Domain Match: {provisioningDomain}', { provisioningDomain }) }}
		<br>
		{{ t('yoomail', 'Email: {email}', { email }) }}<br>
		{{
			t('yoomail', 'IMAP: {user} on {host}:{port} ({ssl} encryption)', {
				user: imapUser,
				host: imapHost,
				port: imapPort,
				ssl: imapSslMode,
		...
		{{
			t('yoomail', 'SMTP: {user} on {host}:{port} ({ssl} encryption)', {
				user: smtpUser,
				host: smtpHost,
				port: smtpPort,
				ssl: smtpSslMode,
		...
		<span v-if="sieveEnabled">
			{{
				t('yoomail', 'Sieve: {user} on {host}:{port} ({ssl} encryption)', {
					user: sieveUser,
					host: sieveHost,
					port: sievePort,
					ssl: sieveSslMode,
