...
<template>
	<SettingsSection
		:name="t('yoomail', 'Mail app')"
		:description="t('yoomail', 'The mail app allows users to read mails on their IMAP accounts.')">
		<p>
			{{
				t(
					'yoomail',
		...
		<p>
			<ButtonVue
				class="config-button"
				:aria-label="t('yoomail', 'Add new config')"
				@click="addNew = true">
				<template #icon>
					<IconAdd :size="20" />
				</template>
				{{ t('yoomail', 'Add new config') }}
			</ButtonVue>
			<ButtonVue
				class="config-button"
				:aria-label="t('yoomail', 'Provision all accounts')"
				@click="provisionAll">
				<template #icon>
					<IconSettings :size="20" />
				</template>
				{{ t('yoomail', 'Provision all accounts') }}
			</ButtonVue>
		...
		<div class="app-description">
			<h3>{{ t('yoomail', 'Allow additional mail accounts') }}</h3>
			<article>
				<p>
					<NcCheckboxRadioSwitch
						...
						{{ t('yoomail', 'Allow additional Mail accounts from User Settings') }}
					</NcCheckboxRadioSwitch>
				</p>
			</article>
		</div>
		<div
			...
			<h3>{{ t('yoomail', 'Enable text processing through LLMs') }}</h3>
			<article>
				<p>
					{{ t('yoomail', 'The Mail app can process user data with the help of the configured large language model and provide assistance features like thread summaries, smart replies and event agendas.') }}
				</p>
				<p>
					<NcCheckboxRadioSwitch
						...
						{{ t('yoomail', 'Enable LLM processing') }}
					</NcCheckboxRadioSwitch>
				</p>
			</article>
		</div>
		<div class="app-description">
			<h3>{{ t('yoomail', 'Enable classification by importance by default') }}</h3>
			<article>
				<p>
					{{ t('yoomail', 'The Mail app can classify incoming emails by importance using machine learning. This feature is enabled by default but can be disabled by default here. Individual users will still be able to toggle the feature for their accounts.') }}
				</p>
				<p>
					<NcCheckboxRadioSwitch
						type="switch"
						...
						{{ t('yoomail', 'Enable classification of important mails by default') }}
					</NcCheckboxRadioSwitch>
				</p>
			</article>
		</div>
		...
		<div class="app-description">
			<h3>
				{{ t('yoomail', 'Microsoft integration') }}
			</h3>
			<article>
				<p>
					{{ t('yoomail', 'Microsoft requires you to access your emails via IMAP using OAuth 2.0 authentication. To do this, you need to register an app with Microsoft Entra ID, formerly known as Microsoft Azure Active Directory.') }}
				</p>
				<p>
					{{ t('yoomail', 'Redirect URI') }}: <code>{{ microsoftOauthRedirectUrl }}</code>
				</p>
				<a :href="microsoftOauthDocs" target="_blank" rel="noopener noreferrer">{{ t('yoomail', 'For more details, please click here to open our documentation.') }}</a>
			</article>
			<MicrosoftAdminOauthSettings :tenant-id="microsoftOauthTenantId" :client-id="microsoftOauthClientId" />
		</div>
		<div class="app-description">
			<h3>{{ t('yoomail', 'User Interface Preference Defaults') }}</h3>
			<article>
				<p>
					{{ t('yoomail', 'These settings are used to pre-configure the user interface preferences they can be overridden by the user in the mail settings') }}
				</p>
			</article>
			<br>
			<article>
				<p>
					{{ t('yoomail', 'Message View Mode') }}
				</p>
				<p>
					<NcCheckboxRadioSwitch
						...
						{{ t('yoomail', 'Show all messages in thread') }}
					</NcCheckboxRadioSwitch>
					<NcCheckboxRadioSwitch
						v-model="layoutMessageView"
						type="radio"
						...
						{{ t('yoomail', 'Show only the selected message') }}
					</NcCheckboxRadioSwitch>
				</p>
			</article>
		</div>
...
const googleOauthClientId = loadState('yoomail', 'google_oauth_client_id', null) ?? undefined
const googleOauthRedirectUrl = loadState('yoomail', 'google_oauth_redirect_url', null)
const microsoftOauthTenantId = loadState('yoomail', 'microsoft_oauth_tenant_id', null) ?? undefined
const microsoftOauthClientId = loadState('yoomail', 'microsoft_oauth_client_id', null) ?? undefined
const microsoftOauthRedirectUrl = loadState('yoomail', 'microsoft_oauth_redirect_url', null)
...
export default {
	...
	data() {
		return {
			...
			microsoftOauthDocs: loadState('yoomail', 'microsoft_oauth_docs'),
			...
			allowNewMailAccounts: loadState('yoomail', 'allow_new_mail_accounts', true),
			isLlmSummaryConfigured: loadState('yoomail', 'enabled_llm_summary_backend'),
			isLlmEnabled: loadState('yoomail', 'llm_processing', true),
			isLlmFreePromptConfigured: loadState('yoomail', 'enabled_llm_free_prompt_backend'),
			layoutMessageView: loadState('yoomail', 'layout_message_view'),
			isImportanceClassificationEnabledByDefault: loadState('yoomail', 'importance_classification_default', true),
		}
	},
	...
	methods: {
		async saveSettings(settings) {
			try {
				...
				showSuccess(t('yoomail', 'Successfully updated config for "{domain}"', { domain: settings.provisioningDomain }))
			} catch (error) {
				showError(t('yoomail', 'Error saving config'))
		...
		async saveNewSettings(settings) {
			try {
				...
				this.addNew = false
				this.resetForm()
				showSuccess(t('yoomail', 'Saved config for "{domain}"', { domain: settings.provisioningDomain }))
			} catch (error) {
				showError(t('yoomail', 'Could not save provisioning setting'))
				logger.error('Could not save provisioning setting', { error })
			}
		},
		...
		async provisionAll() {
			try {
				...
				showSuccess(n('yoomail', 'Successfully provisioned {count} account.', 'Successfully provisioned {count} accounts.', count.count, { count: count.count }))
			} catch (error) {
				showError(t('yoomail', 'There was an error when provisioning accounts.'))
		...
		async deleteProvisioning(id) {
			...
			try {
				...
				showSuccess(t('yoomail', 'Successfully deleted and deprovisioned accounts for "{domain}"', { domain: deleted.provisioningDomain }))
			} catch (error) {
				showError(t('yoomail', 'Error when deleting and deprovisioning accounts for "{domain}"', { domain: deleted.provisioningDomain }))
		...
		async setImportanceClassificationEnabledByDefault(enabledByDefault) {
			try {
				...
				showError(t('yoomail', 'Could not save default classification setting'))
