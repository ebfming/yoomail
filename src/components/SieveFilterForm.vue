...
<template>
	<div class="section">
		...
		<ButtonVue
			type="primary"
			:disabled="loading"
			:aria-label="t('yoomail', 'Save sieve script')"
			@click="saveActiveScript">
			<template #icon>
				<IconLoading v-if="loading" :size="20" />
				<IconCheck v-else :size="20" />
			</template>
			{{ t('yoomail', 'Save sieve script') }}
		</ButtonVue>
	</div>
</template>
		...
		async saveActiveScript() {
			...
			try {
				...
				if (error.response.status === 422) {
					this.errorMessage = t('yoomail', 'The syntax seems to be incorrect:') + ' ' + error.response.data.message
				} else {
