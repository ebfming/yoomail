		...
		<template #list>
			...
			<template v-for="group in menu">
				...
				<div
					...
					<IconAlertTriangle
						:size="18"
						:title="t('yoomail', 'This account cannot connect')" />
					<span @click="showAccountSettings(group.account.id, 'mail-server')">
						{{ t('yoomail', 'Connection failed. Please verify your information and try again') }}
						<NcButton
							:aria-label="t('yoomail', 'Change password')"
							variant="tertiary">{{ t('yoomail', 'Change password') }}</NcButton>
					</span>
				</div>
