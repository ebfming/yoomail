			...
			<div
				...
				<div
					...
					<NcAssistantIcon
						...
						:title="t('yoomail', 'This summary was AI-generated')" />
					{{ isEncrypted ? t('yoomail', 'Encrypted message') : data.summary ? data.summary.trim() : data.previewText.trim() }}
				</div>
			</div>
		</template>
		<template #indicator>
			...
			<IconBullet
				...
				:size="20"
				:aria-hidden="false"
				:aria-label="t('yoomail', 'This message is unread')"
		...
		<template #actions>
			<EnvelopePrimaryActions v-if="!moreActionsOpen && !snoozeOptions" id="primary-actions">
				<ActionButton
					...
					{{
						data.flags.flagged ? t('yoomail', 'Unfavorite') : t('yoomail', 'Favorite')
					}}
				</ActionButton>
				<ActionButton
					v-if="hasSeenAcl"
					...
					{{
						data.flags.seen ? t('yoomail', 'Unread') : t('yoomail', 'Read')
					}}
				</ActionButton>
				<ActionButton
					v-if="hasWriteAcl"
					...
					{{
						isImportant ? t('yoomail', 'Unimportant') : t('yoomail', 'Important')
					}}
				</ActionButton>
			...
			<template v-if="!moreActionsOpen && !snoozeOptions && !quickActionMenu">
				...
				<ActionButton
					...
					{{
						data.flags.$junk ? t('yoomail', 'Mark not spam') : t('yoomail', 'Mark as spam')
					}}
				</ActionButton>
				<ActionButton
					v-if="hasWriteAcl"
				...
				<ActionButton
					...
					<template v-if="layoutMessageViewThreaded">
						{{ t('yoomail', 'Move thread') }}
					</template>
					<template v-else>
						{{ t('yoomail', 'Move Message') }}
					</template>
				</ActionButton>
				<ActionButton
			...
			<template v-if="snoozeOptions">
				...
				<ActionButton
					:aria-label="t('yoomail', 'Set custom snooze')"
					close-after-click
					@click.stop="setCustomSnooze(customSnoozeDateTime)">
					<template #icon>
						<CheckIcon :size="20" />
					</template>
					{{ t('yoomail', 'Set custom snooze') }}
				</ActionButton>
			</template>
			...
			<template v-if="quickActionMenu">
				...
				<ActionButton :close-after-click="true" @click="$emit('open:quick-actions-settings')">
					...
					{{ t('yoomail', 'Manage quick actions') }}
				</ActionButton>
			</template>
		</template>
		<template #tags>
	...
	computed: {
		...
		addresses() {
			...
			if (this.mailbox.specialRole === 'sent' || this.account.sentMailboxId === this.mailbox.databaseId) {
				...
				return recipients.length > 0 ? recipients.join(', ') : t('yoomail', 'Blind copy recipients only')
			}
		 ...
		 */
		subjectForSubtitle() {
			const subject = this.data.subject || this.t('yoomail', 'No subject')
			if (this.draft) {
				return this.t('yoomail', '{markup-start}Draft:{markup-end} {subject}', {
					'markup-start': '<em>',
					'markup-end': '</em>',
		 ...
		 */
		exportMessageLink() {
			return generateUrl('/apps/yoomail/api/messages/{id}/export', {
	...
	methods: {
		...
		async executeQuickAction(action) {
			...
			try {
					...
					switch (step.name) {
						...
						case 'applyTag':
							...
							// usually happens when the tag was deleted in the meantime
								showWarning(t('yoomail', 'Could not apply tag, configured tag not found'))
							}
							break
						...
						case 'moveThread':
							...
							// usually happens when the mailbox was deleted in the meantime
								showWarning(t('yoomail', 'Could not move thread, destination mailbox not found'))
							}
							break
						case 'deleteThread':
							this.onDelete()
				...
				showError(t('yoomail', 'Could not execute quick action'))
				this.quickActionLoading = false
				return
			}
			showSuccess(t('yoomail', 'Quick action executed'))
			this.quickActionLoading = false
		},
		...
		async onDelete() {
			...
			try {
				let result
				if (this.layoutMessageViewThreaded) {
					result = await this.mainStore.deleteThread({
						envelope: this.data,
					})
				} else {
					result = await this.mainStore.deleteMessage({
						id: this.data.databaseId,
					})
				}
		...
		async onArchive() {
			...
			try {
				...
				showError(t('yoomail', 'Could not archive message'))
			}
		},
		...
		async onSnooze(timestamp) {
			...
			try {
				...
				showSuccess(t('yoomail', 'Thread was snoozed'))
			} catch (error) {
				logger.error('could not snooze thread', error)
				showError(t('yoomail', 'Could not snooze thread'))
			}
		},
		...
		async onUnSnooze() {
			...
			try {
				...
				showSuccess(t('yoomail', 'Thread was unsnoozed'))
			} catch (error) {
				logger.error('Could not unsnooze thread', error)
				showError(t('yoomail', 'Could not unsnooze thread'))
			}
		},
