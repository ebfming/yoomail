		...
		<div class="modal-content">
			<div class="left-pane">
				<NcButton
					...
					:aria-label="t('yoomail', 'Maximize composer')"
					:title="largerModal ? t('yoomail', 'Show recipient details') : t('yoomail', 'Hide recipient details')"
					@click="onMaximize">
					<template #icon>
				...
				<NcButton
					...
					variant="tertiary-no-background"
					:aria-label="t('yoomail', 'Minimize composer')"
					:title="t('yoomail', 'Minimize composer')"
					@click="onMinimize">
					<template #icon>
				...
				<KeepAlive>
					<EmptyContent
						v-if="error"
						:name="t('yoomail', 'Error sending your message')"
						class="empty-content"
						role="alert">
						<p>{{ error }}</p>
						<template #action>
							<NcButton variant="tertiary" :aria-label="t('yoomail', 'Go back')" @click="error = undefined">
								{{ t('yoomail', 'Go back') }}
							</NcButton>
							<NcButton variant="tertiary" :aria-label="t('yoomail', 'Retry')" @click="onSend">
								{{ t('yoomail', 'Retry') }}
							</NcButton>
						</template>
					</EmptyContent>
					<EmptyContent
						v-else-if="warning"
						:name="t('yoomail', 'Warning sending your message')"
						class="empty-content"
						role="alert">
						...
						<template #action>
							<NcButton variant="tertiary" :aria-label="t('yoomail', 'Go back')" @click="warning = undefined">
								{{ t('yoomail', 'Go back') }}
							</NcButton>
							<NcButton variant="tertiary" :aria-label="t('yoomail', 'Send anyway')" @click="onForceSend">
								{{ t('yoomail', 'Send anyway') }}
							</NcButton>
						</template>
					</EmptyContent>
	...
	computed: {
		...
		modalTitle() {
			...
			if (this.composerData.forwardFrom) {
				return t('yoomail', 'Forward')
			}
			return t('yoomail', 'New message')
		},
	...
	methods: {
		...
		onDraft(data, { showToast = false } = {}) {
			...
			this.draftsPromise = this.draftsPromise.then(async (id) => {
				...
				try {
					...
					if (showToast) {
						if (this.composerMessage.type === 'outbox') {
							showSuccess(t('yoomail', 'Message saved'))
						} else {
							showSuccess(t('yoomail', 'Draft saved'))
						}
					}
					...
					if (showToast) {
						if (this.composerMessage.type === 'outbox') {
							showError(t('yoomail', 'Failed to save message'))
						} else {
							showError(t('yoomail', 'Failed to save draft'))
						}
					}
				} finally {
		...
		async onSend(data, force = false) {
			...
			try {
				...
				if (!force && data.attachments.length === 0) {
					...
					const wordAttachment = t('yoomail', 'attachment').toLowerCase()
					const wordAttached = t('yoomail', 'attached').toLowerCase()
				...
				this.error = await matchError(error, {
					[NoSentMailboxConfiguredError.getName()]() {
						...
						return t('yoomail', 'No "sent" folder configured. Please pick one in the account settings.')
					},
					[ManyRecipientsError.getName()]() {
						logger.error('could not send message', { error })
						return t('yoomail', 'You are trying to send to many recipients in To and/or Cc. Consider using Bcc to hide recipient addresses.')
					},
					default(error) {
						logger.error('could not send message', { error })
					},
				...
				this.warning = await matchError(error, {
					[SubjectMissingError.getName()]() {
						...
						return t('yoomail', 'Your message has no subject. Do you want to send it anyway?')
					},
					[AttachmentMissingError.getName()]() {
						logger.info('showing the did you forgot an attachment warning', { error })
						return t('yoomail', 'You mentioned an attachment. Did you forget to add it?')
					},
					default(error) {
		...
		async discardDraft() {
			...
			try {
				...
				showSuccess(t('yoomail', 'Message discarded'))
			} catch (error) {
				logger.error('Could not discard draft', { error })
				showError(t('yoomail', 'Could not discard message'))
			}
		},
