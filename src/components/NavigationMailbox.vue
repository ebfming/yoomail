	...
	<AppNavigationItem
		...
		<template #actions>
			...
			<ActionButton
				...
				:name="t('yoomail', 'Mark all as read')"
			...
			<ActionButton
				...
				{{ t('yoomail', 'Repair folder') }}
			</ActionButton>
			<ActionButton
				v-if="debug && !account.isUnified && mailbox.specialRole !== 'flagged'"
				:name="t('yoomail', 'Clear cache')"
				:disabled="clearingCache"
				@click="clearCache">
				<template #icon>
					<IconFolderSync :size="20" />
				</template>
				{{ t('yoomail', 'Clear locally cached data, in case there are issues with synchronization.') }}
			</ActionButton>
		...
		statsText() {
			if (this.mailboxStats && 'total' in this.mailboxStats && 'unread' in this.mailboxStats) {
				if (this.mailboxStats.unread === 0) {
					return n('yoomail', '{total} message', '{total} messages', this.mailboxStats.total, {
	...
	methods: {
		...
		async renameMailbox() {
			...
			try {
				...
				showInfo(t('yoomail', 'An error occurred, unable to rename the mailbox.'))
		...
		async repair() {
			...
			try {
				...
				if (error.response?.status === 429) {
					...
					showError(t('yoomail', 'Please wait 10 minutes before repairing again'))
				} else {
					throw error
				}
			}
