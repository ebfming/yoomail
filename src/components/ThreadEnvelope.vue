	...
	<div
		...
		<div
			...
			<span class="envelope__follow-up-header__date">
				{{ t('yoomail', "You've sent this message on {date}", { date: formattedSentAt }) }}
			</span>
			<div class="envelope__follow-up-header__actions">
				<NcButton @click="onDisableFollowUpReminder">
					{{ t('yoomail', 'Disable reminder') }}
				</NcButton>
			</div>
		</div>
			...
			<div
				...
				<div class="envelope__header__left__sender-subject-tags">
					...
					<div v-if="showSubline" class="subline">
						<span class="preview">
							{{ isEncrypted ? t('yoomail', 'Encrypted message') : envelope.previewText }}
						</span>
					</div>
					<div class="tagline">
						<div
				...
				<template v-if="expanded">
					...
					<NcActions :inline="inlineMenuSize">
						<NcActionButton
							...
							<template #icon>
								<ReplyAllIcon
									...
									:title="t('yoomail', 'Reply all')"
									:size="20" />
								<ReplyIcon
									v-else
									:title="t('yoomail', 'Reply')"
									:size="20" />
							</template>
							{{ t('yoomail', 'Reply') }}
						</NcActionButton>
						<NcActionButton
							...
							<template #icon>
								<ReplyIcon
									:title="t('yoomail', 'Reply to sender only')"
									:size="20" />
							</template>
							{{ t('yoomail', 'Reply to sender only') }}
						</NcActionButton>
						<NcActionButton
							v-if="hasWriteAcl && (inlineMenuSize >= 2 || !moreActionsOpen)"
							type="tertiary-no-background"
							class="action--primary"
							:aria-label="envelope.flags.flagged ? t('yoomail', 'Mark as unfavorite') : t('yoomail', 'Mark as favorite')"
							:close-after-click="true"
							@click.prevent="onToggleFlagged">
							<template #icon>
								<IconFavorite
									v-if="showFavoriteIconVariant"
									:title="t('yoomail', 'Mark as unfavorite')"
									:size="20" />
								<StarOutline
									v-else
									:title="t('yoomail', 'Mark as favorite')"
									:size="20" />
							</template>
							{{ envelope.flags.flagged ? t('yoomail', 'Mark as unfavorite') : t('yoomail', 'Mark as favorite') }}
						</NcActionButton>
						<NcActionButton
							v-if="hasSeenAcl && (inlineMenuSize >= 3 || !moreActionsOpen)"
							type="tertiary-no-background"
							class="action--primary"
							:aria-label="envelope.flags.seen ? t('yoomail', 'Mark as unread') : t('yoomail', 'Mark as read')"
							:close-after-click="true"
							@click.prevent="onToggleSeen">
							<template #icon>
								<EmailRead
									v-if="showImportantIconVariant"
									:title="t('yoomail', 'Mark as unread')"
									:size="20" />
								<EmailUnread
									v-else
									:title="t('yoomail', 'Mark as read')"
									:size="20" />
							</template>
							{{ envelope.flags.seen ? t('yoomail', 'Mark as unread') : t('yoomail', 'Mark as read') }}
						</NcActionButton>
						<NcActionButton
							v-if="showArchiveButton && hasArchiveAcl && (inlineMenuSize >= 4 || !moreActionsOpen)"
							:close-after-click="true"
							:disabled="disableArchiveButton"
							:aria-label="t('yoomail', 'Archive message')"
							type="tertiary-no-background"
							@click.prevent="onArchive">
							<template #icon>
								<ArchiveIcon
									:title="t('yoomail', 'Archive message')"
									:size="20" />
							</template>
							{{ t('yoomail', 'Archive message') }}
						</NcActionButton>
						<NcActionButton
							v-if="hasDeleteAcl && (inlineMenuSize >= 5 || !moreActionsOpen)"
							:close-after-click="true"
							:aria-label="t('yoomail', 'Delete message')"
							type="tertiary-no-background"
							@click.prevent="onDelete">
							<template #icon>
								<DeleteIcon
									:title="t('yoomail', 'Delete message')"
									:size="20" />
							</template>
							{{ t('yoomail', 'Delete message') }}
						</NcActionButton>
						<MenuEnvelope
		...
		<div v-if="expanded && showRecipients" class="envelope__recipients">
			<div v-if="envelope.from && envelope.from.length" class="recipients">
				<span class="recipients__label">{{ t('yoomail', 'From:') }}</span>
				<RecipientBubble
			...
			<div v-if="envelope.to && envelope.to.length" class="recipients">
				<span class="recipients__label">{{ t('yoomail', 'To:') }}</span>
			...
			<div v-if="envelope.cc && envelope.cc.length" class="recipients">
				<span class="recipients__label">{{ t('yoomail', 'Cc:') }}</span>
				<div class="recipients__list">
					<RecipientBubble
			...
			<div v-if="envelope.bcc && envelope.bcc.length" class="recipients">
				<span class="recipients__label">{{ t('yoomail', 'Bcc:') }}</span>
				<div class="recipients__list">
					<RecipientBubble
		...
		<Error
			v-else-if="error"
			:error="error.message || t('yoomail', 'Not found')"
			message=""
			:data="error"
			:auto-margin="true"
			role="alert" />
		<ConfirmModal
			...
			:confirm-text="t('yoomail', 'Unsubscribe')"
			:title="t('yoomail', 'Unsubscribe via link')"
			@cancel="showListUnsubscribeConfirmation = false"
			@confirm="unsubscribeViaOneClick">
			{{ t('yoomail', 'Unsubscribing will stop all messages from the mailing list {sender}', { sender: from }) }}
		</ConfirmModal>
		<ConfirmModal
		...
		<ConfirmModal
			...
			:confirm-text="t('yoomail', 'Send unsubscribe email')"
			:title="t('yoomail', 'Unsubscribe via email')"
...
export default {
	...
	data() {
		return {
			...
			isInternal: true,
			enabledFreePrompt: loadState('yoomail', 'llm_freeprompt_available', false),
	...
	computed: {
		...
		smimeHeading() {
			if (this.smimeData.isEncrypted) {
				return t('yoomail', 'Encrypted & verified ')
			}

			if (this.smimeData.signatureIsValid) {
				return t('yoomail', 'Signature verified')
			}

			return t('yoomail', 'Signature unverified ')
		},

		smimeMessage() {
			if (this.smimeData.isEncrypted) {
				return t('yoomail', 'This message was encrypted by the sender before it was sent.')
			}

			if (this.smimeData.signatureIsValid) {
				return t('yoomail', 'This message contains a verified digital S/MIME signature. The message wasn\'t changed since it was sent.')
			}

			return t('yoomail', 'This message contains an unverified digital S/MIME signature. The message might have been changed since it was sent or the certificate of the signer is untrusted.')
		},
		...
		replyButtonLabel() {
			if (this.showFollowUpHeader) {
				return t('yoomail', 'Follow up')
			}

			if (this.hasMultipleRecipients) {
				return t('yoomail', 'Reply all')
			}

			return t('yoomail', 'Reply')
		},
	},
	...
	methods: {
		...
		async onDelete() {
			...
			try {
				const result = await this.mainStore.deleteMessage({
					id: this.envelope.databaseId,
				})
				if (result?.remoteMissing === true) {
					showWarning(t('yoomail', result?.message || 'The local message was deleted. The remote message may already have been deleted.'))
				}
			} catch (error) {
				const message = error?.response?.data?.message
				showError(message || await matchError(error, {
					[NoTrashMailboxConfiguredError.getName()]() {
						return t('yoomail', 'No trash folder configured')
					},
					default(error) {
						logger.error('could not delete message', error)
						return t('yoomail', 'Could not delete message')
					},
				}))
			}
		},
		...
		async onArchive() {
			...
			try {
				...
				return t('yoomail', 'Could not archive message')
			}
		},
		...
		async unsubscribeViaOneClick() {
			try {
				...
				showSuccess(t('yoomail', 'Unsubscribe request sent'))
			} catch (error) {
				logger.error('Could not one-click unsubscribe', { error })
				showError(t('yoomail', 'Could not unsubscribe from mailing list'))
			} finally {
		...
		onOpenTranslationModal() {
			try {
				...
				showError(t('yoomail', 'Please wait for the message to load'))
			}
		},
		...
		async onShowSourceModal() {
			if (this.rawMessage.length === 0) {
				const resp = await axios.get(generateUrl('/apps/yoomail/api/messages/{id}/source', {
					id: this.envelope.databaseId,
				}))
