...
<template>
	<NcDialog
		...
		:name="t('yoomail', 'Translate message')"
		size="large"
		...
		<template v-if="isMounted" #default>
			<div class="translate-dialog__wrapper">
				<NcSelect
					...
					input-id="from"
					label="name"
					:aria-label-combobox="t('yoomail', 'Source language to translate from')"
					:placeholder="t('yoomail', 'Translate from')"
				...
				<NcSelect
					...
					:placeholder="t('yoomail', 'Translate to')"
				...
				<NcButton
					...
					{{ isLoading ? t('yoomail', 'Translating') : t('yoomail', 'Translate') }}
				</NcButton>
			</div>
			<div class="translate-dialog__ai-note">
				<WarningIcon class="translate-dialog__ai-note-icon" />
				<span>
					{{ t('yoomail', 'This translation is generated using AI and may contain inaccuracies') }}
				</span>
			</div>
		...
		<template v-if="translatedMessage" #actions>
			<NcButton @click="handleCopyTranslation">
				...
				{{ t('yoomail', 'Copy translated text') }}
			</NcButton>
		</template>
	</NcDialog>
</template>
	...
	methods: {
		handleTranslate() {
			if (!this.selectedFrom || !this.selectedTo) {
				showError(t('yoomail', 'Please select languages to translate to and from'))
				return
			}
		...
		async translateMessage() {
			try {
				...
				showError(error.response?.data?.ocs?.data?.message ?? t('yoomail', 'The message could not be translated'))
			} finally {
		...
		async handleCopyTranslation() {
			try {
				...
				showSuccess(t('yoomail', 'Translation copied to clipboard'))
			} catch (error) {
				logger.error('could not copy translation to clipboard', { error })
				showError(t('yoomail', 'Translation could not be copied'))
			}
		},
	},
}
