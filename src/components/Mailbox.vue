...
<template>
	<div
		...
		<Error
			v-if="error"
			:error="t('yoomail', 'Could not open folder')"
			message=""
			role="alert" />
		<LoadingSkeleton v-else-if="loadingEnvelopes" :number-of-lines="20" />
		<Loading
			v-else-if="loadingCacheInitialization"
			:hint="t('yoomail', 'Loading messages …')"
			:slow-hint="t('yoomail', 'Indexing your messages. This can take a bit longer for larger folders.')" />
...
export default {
	...
	methods: {
			...
			switch (e.srcKey) {
				...
				case 'arch': {
					...
					if (account.archiveMailboxId === null) {
						showWarning(t('yoomail', 'To archive a message please configure an archive folder in account settings'))
						return
					}

					if (!this.hasArchiveAcl()) {
						showWarning(t('yoomail', 'You are not allowed to move this message to the archive folder and/or delete this message from the current folder'))
						return
					}
				...
				case 'unseen':
					...
					if (!this.hasSeenAcl()) {
						showWarning(t('yoomail', 'Your IMAP server does not support storing the seen/unseen state.'))
						return
					}
					...
					try {
						...
						showError(t('yoomail', 'Could not mark message as seen/unseen'))
					}
					break
				default:
		...
		getLabelForGroup(group) {
			switch (group) {
				case 'lastHour':
					return t('yoomail', 'Last hour')
				case 'today':
					return t('yoomail', 'Today')
				case 'yesterday':
					return t('yoomail', 'Yesterday')
				case 'lastWeek':
					return t('yoomail', 'Last week')
				case 'lastMonth':
					return t('yoomail', 'Last month')
				default:
					return group
			}
		},
