...
<template>
	<div class="section">
		<p>{{ t('yoomail', 'Take control of your email chaos. Filters help you to prioritize what matters and eliminate clutter.') }}</p>
		<div v-if="loading" class="filter-list__loading">
			<NcLoadingIcon />
			<p>{{ t('yoomail', 'Hang tight while the filters load') }}</p>
		</div>
		<div v-else class="filter-list">
			<ul>
				<NcListItem
					...
					<template #subname>
						<span v-if="filter.enable">{{ t('yoomail', 'Filter is active') }}</span>
						<span v-else>{{ t('yoomail', 'Filter is not active') }}</span>
					</template>
					<template #actions>
			...
			<NcButton
				...
				variant="primary"
				:aria-label="t('yoomail', 'New filter')"
				@click.prevent.stop="createFilter">
				{{ t('yoomail', 'New filter') }}
			</NcButton>
		</div>
...
export default {
	...
	methods: {
		...
		async updateFilter(filter) {
			...
			try {
				await this.mailFilterStore.store(this.account.id).then(() => {
					showSuccess(t('yoomail', 'Filter saved'))
				})
				await this.mainStore.fetchActiveSieveScript({ accountId: this.account.id })
			} catch (e) {
				logger.error(e)
				showError(t('yoomail', 'Could not save filter'))
			} finally {
				this.loading = false
			}
		},
		...
		async deleteFilter(filter) {
			...
			try {
				await this.mailFilterStore.store(this.account.id).then(() => {
					showSuccess(t('yoomail', 'Filter deleted'))
				})
			} catch (e) {
				logger.error(e)
				showError(t('yoomail', 'Could not delete filter'))
			} finally {
				this.loading = false
			}
