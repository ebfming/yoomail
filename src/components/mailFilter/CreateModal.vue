...
<template>
	<NcModal
		...
		<form class="modal__content" @submit.prevent="createFilter">
			<h2>{{ t('yoomail', 'Create a new mail filter') }}</h2>

			<p class="intro-text">
				{{ t('yoomail', 'Choose the headers you want to use to create your filter. In the next step, you will be able to refine the filter conditions and specify the actions to be taken on messages that match your criteria.') }}
			</p>
	...
	methods: {
		prepareHeaders() {
			...
			this.headers.push({
				...
				label: t('yoomail', 'Subject') + ': ' + this.envelope.subject,
				enable: true,
			...
			for (const from of this.envelope.from) {
				this.headers.push({
					...
					value: from.email,
					label: t('yoomail', 'Sender') + ': ' + from.email,
					enable: true,
			...
			for (const to of this.envelope.to) {
				this.headers.push({
					...
					value: to.email,
					label: t('yoomail', 'Recipient') + ': ' + to.email,
					enable: true,
