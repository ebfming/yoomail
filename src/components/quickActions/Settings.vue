...
<template>
	<div class="quick-actions-settings">
		<div v-if="quickActions.length === 0">
			{{ t('yoomail', 'No quick actions yet.') }}
		</div>
		...
		<NcButton class="add-quick-action" variant="primary" @click="openEditModal()">
			{{ t('yoomail', 'Add quick action') }}
		</NcButton>
		<NcModal v-if="editModal" :name="modalName" @close="closeEditModal">
			<h2 class="modal-name" v-text="modalName" />
			<div class="modal-content">
				<NcTextField v-model="localAction.name" :label="t('yoomail', 'Quick action name')" />
				<h3>{{ t('yoomail', 'Do the following actions') }}</h3>
				...
				<NcActions :menu-name="t('yoomail', 'Add another action')">
					<template #icon>
						<PlusIcon :size="20" />
					</template>
...
export default {
	...
	computed: {
		...
		modalName() {
			return this.editMode ? this.t('yoomail', 'Edit quick action') : this.t('yoomail', 'Add quick action')
		},
	...
	methods: {
		async deleteQuickAction(id) {
			try {
				...
				showSuccess(t('yoomail', 'Quick action deleted'))
			} catch (error) {
				logger.error('Could not delete quick action', {
					error,
				})
				showError(t('yoomail', 'Failed to delete quick action'))
			}
		},
		...
		async saveQuickAction() {
			...
			if (this.editMode) {
				...
				try {
					...
					showError(t('yoomail', 'Failed to update quick action'))
					return
				}
				for (const [index, action] of this.actions.entries()) {
					if (action?.id !== null && action?.id !== undefined) {
						try {
							...
							showError(t('yoomail', 'Failed to update step in quick action'))
						}
					} else {
				...
				showSuccess(t('yoomail', 'Quick action updated'))
			} else {
				let quickAction
				const createdSteps = []
				try {
					...
					showError(t('yoomail', 'Failed to create quick action'))
					return
				}
				try {
					...
					showError(t('yoomail', 'Failed to add steps to quick action'))
					this.closeEditModal()
				}
				this.localAction = quickAction
				showSuccess(t('yoomail', 'Quick action created'))
			}
		...
		async deleteAction(item) {
			...
			if (item.id) {
				try {
					...
					showError(t('yoomail', 'Failed to delete action step'))
					return
				}
			}
		},
