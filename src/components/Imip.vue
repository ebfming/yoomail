...
<template>
	<div class="imip">
		...
		<div
			...
			<span>{{ t('yoomail', 'This event was cancelled') }}</span>
		</div>
		...
		<div v-if="showMoreOptions" class="imip__more-options">
			...
			<div class="imip__more-options__row imip__more-options__row--comment">
				<label for="commentFieldId">{{ t('yoomail', 'Comment') }}</label>
		...
		<template v-if="isRequest && userIsAttendee">
			<div
				...
				<NcButton
					variant="tertiary"
					:disabled="loading"
					:aria-label="t('yoomail', 'Tentatively accept')"
					@click="acceptTentatively">
					{{ t('yoomail', 'Tentatively accept') }}
				</NcButton>
				<NcButton
					v-if="!showMoreOptions"
					variant="tertiary"
					:disabled="loading"
					:aria-label="t('yoomail', 'More options')"
					@click="showMoreOptions = true">
					{{ t('yoomail', 'More options') }}
				</NcButton>
				<NcLoadingIcon v-if="loading" />
			</div>
			<p v-else-if="!eventIsInFuture" class="imip__actions imip__actions--hint">
				{{ t('yoomail', 'This message has an attached invitation but the invitation dates are in the past') }}
			</p>
		</template>
		<div v-if="!userIsAttendee" class="imip__actions imip__actions--hint">
			{{ t('yoomail', 'This message has an attached invitation but the invitation does not contain a participant that matches any configured mail account address') }}
		</div>
	</div>
</template>
	...
	computed: {
		...
		replyStatusMessage() {
			...
			if (!attendees || attendees.length !== 1) {
				...
				return this.t('yoomail', 'This event was updated')
			}
			...
			if (partStat === ACCEPTED) {
				return this.t('yoomail', '{attendeeName} accepted your invitation', {
					attendeeName: name,
				})
			} else if (partStat === TENTATIVE) {
				return this.t('yoomail', '{attendeeName} tentatively accepted your invitation', {
					attendeeName: name,
				})
			} else if (partStat === DECLINED) {
				return this.t('yoomail', '{attendeeName} declined your invitation', {
					attendeeName: name,
				})
			}

			return this.t('yoomail', '{attendeeName} reacted to your invitation', {
				attendeeName: name,
			})
		},
		...
		async saveEventWithParticipationStatus(status) {
			...
			if (!this.isExistingEvent) {
				try {
					...
					showError(this.t('yoomail', 'Failed to save your participation status'))
