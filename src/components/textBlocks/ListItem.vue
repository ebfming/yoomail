		...
		<NcListItem class="text-block-list-item" :active="picked" @click="handleListItemClick">
			...
			<template v-if="!shared" #extra-actions>
				<AccountMultiple
					...
					:title="t('yoomail', 'Shared')"
					:size="20" />
				<NcActionButton :title="t('yoomail', 'Edit {title}', { title: textBlock.title })" @click="editModalOpen = true">
					<template #icon>
						<IconPencil :size="20" />
					</template>
				</NcActionButton>
				<NcActionButton
					v-if="!isViewMode"
					icon="icon-delete"
					:title="t('yoomail', 'Delete {title}', { title: textBlock.title })"
					@click="deleteTextBlock()" />
			</template>
		</NcListItem>
		<NcDialog
			:open.sync="editModalOpen"
			:name="t('yoomail', 'Edit text block')"
			size="normal"
			:is-form="true">
			<p v-if="shared">
				{{ localTextBlock.title }}
			</p>
			<NcInputField v-else v-model="localTextBlock.title" :label="t('yoomail', 'Title of the text block')" />
			<TextEditor
			...
			<template v-if="!shared">
				...
				<NcSelectUsers
					...
					:placeholder="t('yoomail', 'Search for users or groups')"
					:label-outside="true"
					:loading="loading"
					:options="options"
	...
	methods: {
		async deleteTextBlock() {
			await this.mainStore.deleteTextBlock({ id: this.textBlock.id }).then(() => {
				showSuccess(t('yoomail', 'Text block deleted'))
			}).catch((error) => {
				logger.error('failed to delete text block', { error })
				showError(t('yoomail', 'Failed to delete text block'))
			})
		},
		...
		async shareTextBlock(sharee) {
			try {
				...
				showSuccess(t('yoomail', 'Text block shared with {sharee}', { sharee: sharee.shareWith }))
				this.share = null
			} catch (error) {
				logger.error('failed to share text block', { error })
				showError(t('yoomail', 'Failed to share text block with {sharee}', { sharee: sharee.shareWith }))
			}
		},
		...
		async removeShare(sharee) {
			try {
				...
				showSuccess(t('yoomail', 'Share deleted for {name}', { name: sharee.shareWith }))
			} catch (error) {
				logger.error('failed to delete text block share', { error })
				showError(t('yoomail', 'Failed to delete share with {name}', { name: sharee.shareWith }))
			}
		},
		...
		async saveTextBlock() {
			...
			try {
				...
				showError(t('yoomail', 'Failed to save text block'))
