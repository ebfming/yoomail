<!--
  - SPDX-FileCopyrightText: 2026 Nextcloud GmbH and Nextcloud contributors
  - SPDX-License-Identifier: AGPL-3.0-or-later
-->

<template>
	<NcModal
		v-if="view === 'main'"
		size="normal"
		label-id="delegation-modal-title"
		@close="$emit('close')">
		<div class="delegation-modal">
			<h2 id="delegation-modal-title">
				{{ t('yoomail', 'Delegation') }}
			</h2>

			<div class="delegation-modal__section">
				<p class="delegation-modal__description">
					{{ t('yoomail', 'Allow users to send, receive, and delete mail on your behalf') }}
				</p>
				<div v-if="loading" class="filter-list__loading">
					<NcLoadingIcon />
				</div>
				<ul v-else>
					<NcListItem
						v-for="user in delegates"
						:key="user.userId"
						:name="user.displayName || user.userId">
						<template #icon>
							<NcAvatar
								disable-menu
								:size="34"
								:user="user.userId" />
						</template>
						<template #extra-actions>
							<NcButton
								:title="t('yoomail', 'Revoke access')"
								:aria-label="t('yoomail', 'Revoke access')"
								variant="tertiary-no-background"
								@click="confirmRevoke(user)">
								<template #icon>
									<IconClose :size="20" />
								</template>
							</NcButton>
						</template>
					</NcListItem>
				</ul>
				<NcButton
					wide
					variant="secondary"
					@click="openAddDelegate">
					<template #icon>
						<IconPlus :size="20" />
					</template>
					{{ t('yoomail', 'Add delegate') }}
				</NcButton>
			</div>
		</div>
	</NcModal>

	<NcDialog
		v-else-if="view === 'add'"
		class="add-delegates-dialog"
		:open="view === 'add'"
		:name="t('yoomail', 'Add delegate')"
		:buttons="addDelegateButtons"
		@closing="closeDialog">
		<NcSelectUsers
			v-model="selectedUser"
			class="add-delegates-dialog__select"
			:input-label="t('yoomail', 'Select a user')"
			:options="userSuggestions"
			:loading="searchLoading"
			:placeholder="t('yoomail', 'Select a user')"
			@search="onSearch" />
		<p class="add-delegates-dialog__description">
			{{ t('yoomail', 'They will be able to send, receive, and delete mail on your behalf') }}
		</p>
	</NcDialog>

	<NcDialog
		v-else-if="view === 'revoke'"
		class="revoke-dialog"
		:open="view === 'revoke'"
		:name="t('yoomail', 'Revoke access?')"
		:buttons="revokeButtons"
		@closing="closeDialog">
		<p class="revoke-dialog__text">
			{{ revokeText }}
		</p>
	</NcDialog>
</template>

<script>
import IconCheck from '@mdi/svg/svg/check.svg?raw'
import { getCurrentUser } from '@nextcloud/auth'
import axios from '@nextcloud/axios'
import { showError, showSuccess } from '@nextcloud/dialogs'
import { generateOcsUrl } from '@nextcloud/router'
import { ShareType } from '@nextcloud/sharing'
import { NcAvatar, NcButton, NcDialog, NcListItem, NcLoadingIcon, NcModal, NcSelectUsers } from '@nextcloud/vue'
import debounce from 'lodash/fp/debounce.js'
import IconClose from 'vue-material-design-icons/Close.vue'
import IconPlus from 'vue-material-design-icons/Plus.vue'
import logger from '../logger.js'
import { delegate, fetchDelegatedUsers, unDelegate } from '../service/DelegationService.js'

export default {
	name: 'DelegationModal',
	components: {
		NcAvatar,
		NcButton,
		NcDialog,
		NcListItem,
		NcModal,
		NcSelectUsers,
		NcLoadingIcon,
		IconClose,
		IconPlus,
	},

	props: {
		account: {
			type: Object,
			required: true,
		},
	},

	data() {
		return {
			view: 'main',
			delegates: [],
			loading: true,
			revokeUser: null,
			selectedUser: null,
			userSuggestions: [],
			searchLoading: false,
			delegating: false,
		}
	},

	computed: {
		addDelegateButtons() {
			return [
				{
					label: t('yoomail', 'Cancel'),
					type: 'tertiary',
					disabled: this.delegating,
					callback: () => { this.closeDialog() },
				},
				{
					label: t('yoomail', 'Delegate access'),
					type: 'primary',
					icon: IconCheck,
					disabled: !this.selectedUser || this.delegating,
					callback: async () => { await this.addDelegate() },
				},
			]
		},

		revokeButtons() {
			return [
				{
					label: t('yoomail', 'Cancel'),
					type: 'tertiary',
					callback: () => { this.closeDialog() },
				},
				{
					label: t('yoomail', 'Revoke'),
					type: 'error',
					callback: async () => { await this.revokeDelegate() },
				},
			]
		},

		revokeText() {
			if (!this.revokeUser) {
				return ''
			}
			return t('yoomail', '{userId} will no longer be able to act on your behalf', { userId: this.revokeUser.displayName || this.revokeUser.userId })
		},
	},

	async mounted() {
		await this.fetchDelegates()
		this.loading = false
	},

	methods: {
		async fetchDelegates() {
			try {
				this.delegates = await fetchDelegatedUsers(this.account.accountId)
			} catch (error) {
				logger.error('Could not fetch delegates', { error })
				showError(t('yoomail', 'Could not fetch delegates'))
			}
		},

		openAddDelegate() {
			this.view = 'add'
		},

		confirmRevoke(user) {
			this.revokeUser = user
			this.view = 'revoke'
		},

		onSearch(query) {
			this.debounceGetSuggestions(query.trim())
		},

		debounceGetSuggestions: debounce(300, function(...args) {
			this.getSuggestions(...args)
		}),

		async getSuggestions(search) {
			if (!search) {
				this.userSuggestions = []
				return
			}

			this.searchLoading = true
			try {
				const request = await axios.get(generateOcsUrl('apps/files_sharing/api/v1/sharees'), {
					params: {
						format: 'json',
						itemType: 'file',
						search,
						shareTypes: [ShareType.User],
					},
				})

				const data = request.data.ocs.data
				const exact = request.data.ocs.data.exact

				const rawSuggestions = exact.users.concat(data.users)
				const currentUserId = getCurrentUser().uid
				const delegateIds = this.delegates.map((d) => d.userId)

				this.userSuggestions = rawSuggestions
					.map((result) => ({
						id: result.value.shareWith,
						displayName: result.name || result.label,
						subname: result.value.shareWith,
						user: result.value.shareWith,
					}))
					.filter((u) => u.id !== currentUserId && !delegateIds.includes(u.id))
			} catch (error) {
				logger.error('Error fetching user suggestions', { error })
			} finally {
				this.searchLoading = false
			}
		},

		async addDelegate() {
			if (!this.selectedUser) {
				return
			}

			this.delegating = true
			try {
				const delegation = await delegate(this.account.accountId, this.selectedUser.id)
				this.delegates.push({ ...delegation, displayName: this.selectedUser.displayName })
				showSuccess(t('yoomail', 'Delegated access to {displayName}', { displayName: this.selectedUser.displayName || this.selectedUser.id }))
			} catch (error) {
				logger.error('Could not delegate access', { error })
				showError(t('yoomail', 'Could not delegate access'))
			} finally {
				this.delegating = false
				this.closeDialog()
			}
		},

		async revokeDelegate() {
			if (!this.revokeUser) {
				return
			}

			try {
				await unDelegate(this.account.accountId, this.revokeUser.userId)
				this.delegates = this.delegates.filter((d) => d.userId !== this.revokeUser.userId)
				showSuccess(t('yoomail', 'Revoked access for {displayName}', { displayName: this.revokeUser.displayName }))
			} catch (error) {
				logger.error('Could not revoke delegation', { error })
				showError(t('yoomail', 'Could not revoke delegation'))
			}
		},

		closeDialog() {
			this.view = 'main'
		},
	},
}
</script>

<style lang="scss" scoped>
.delegation-modal {
	padding: var(--default-grid-baseline) calc(var(--default-grid-baseline) * 3) calc(var(--default-grid-baseline) * 3);
	h2 {
		margin: 0;
		text-align: center;
	}
	&__section {
		margin-top: calc(var(--default-grid-baseline) * 3);
	}

	&__description {
		color: var(--color-text-maxcontrast);
		margin-bottom: calc(var(--default-grid-baseline) * 2);
	}
}

.add-delegates-dialog{
	&__description{
		color: var(--color-text-maxcontrast);
		padding: calc(var(--default-grid-baseline) * 2) 0;
	}
	&__select{
		width: 100%;
	}
}

.revoke-dialog {
	&__text{
		padding-bottom: calc(var(--default-grid-baseline) * 2);
	}
}
</style>
