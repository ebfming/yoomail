...
<template>
	<div>
			...
			<div v-if="selectMode" key="multiselect-header" class="multiselect-header">
				<div class="action-buttons">
					<NcButton
						...
						variant="tertiary"
						:title="n('yoomail', 'Mark {number} read', 'Mark {number} read', selection.length, { number: selection.length })"
					...
					<NcButton
						...
						variant="tertiary"
						:title="n('yoomail', 'Mark {number} unread', 'Mark {number} unread', selection.length, { number: selection.length })"
					...
					<NcButton
						...
						variant="tertiary"
						:title="n('yoomail', 'Mark {number} as important', 'Mark {number} as important', selection.length, { number: selection.length })"
					...
					<NcButton
						...
						variant="tertiary"
						:title="n('yoomail', 'Mark {number} as unimportant', 'Mark {number} as unimportant', selection.length, { number: selection.length })"
					...
					<NcButton
						...
						variant="tertiary"
						:title="n('yoomail', 'Unfavorite {number}', 'Unfavorite {number}', selection.length, { number: selection.length })"
					...
					<NcButton
						...
						variant="tertiary"
						:title="n('yoomail', 'Favorite {number}', 'Favorite {number}', selection.length, { number: selection.length })"
					...
					<NcButton
						variant="tertiary"
						:title="n('yoomail', 'Unselect {number}', 'Unselect {number}', selection.length, { number: selection.length })"
				...
				<Actions class="app-content-list-item-menu" menu-align="right">
					<ActionButton
						...
						{{ n('yoomail', 'Mark {number} as spam', 'Mark {number} as spam', selection.length, { number: selection.length }) }}
					</ActionButton>
					<ActionButton
						...
						{{ n('yoomail', 'Mark {number} as not spam', 'Mark {number} as not spam', selection.length, { number: selection.length }) }}
					</ActionButton>
					<ActionButton :close-after-click="true" @click.prevent="onOpenTagModal">
						<template #icon>
							<TagIcon :size="20" />
						</template>
						{{ n('yoomail', 'Edit tags for {number}', 'Edit tags for {number}', selection.length, { number: selection.length }) }}
					</ActionButton>
					<ActionButton v-if="!account.isUnified" :close-after-click="true" @click.prevent="onOpenMoveModal">
						<template #icon>
							<OpenInNewIcon :size="20" />
						</template>
						{{ n('yoomail', 'Move {number} thread', 'Move {number} threads', selection.length, { number: selection.length }) }}
					</ActionButton>
					<ActionButton :close-after-click="true" @click.prevent="forwardSelectedAsAttachment">
						<template #icon>
							<ShareIcon :size="20" />
						</template>
						{{ n('yoomail', 'Forward {number} as attachment', 'Forward {number} as attachment', selection.length, { number: selection.length }) }}
					</ActionButton>
				</Actions>
			</div>
		</transition>
		...
		<NcDialog
			...
			:name="t('yoomail', 'Manage quick actions')"
	...
	methods: {
		...
		async deleteAllSelected() {
			...
			await Promise.all(this.selectedEnvelopes.map(async (envelope) => {
				...
				const result = await this.mainStore.deleteThread({
					envelope,
				})
