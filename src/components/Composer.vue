	...
	<div class="message-composer">
		...
		<div class="composer-fields composer-fields__from mail-account">
			...
			<div class="composer-fields--custom">
				<NcSelect
					...
					:searchable="false"
					:placeholder="t('yoomail', 'Select account')"
					:aria-label-combobox="t('yoomail', 'Select account')"
		...
		<div class="composer-fields">
			...
			<div class="composer-fields--custom">
				<NcSelect
					...
					:taggable="true"
					:aria-label-combobox="t('yoomail', 'Select recipient')"
					...
					<template #search="{ events, attributes }">
						<input
							:placeholder="t('yoomail', 'Contact or email address …')"
							type="search"
							class="vs__search"
							v-bind="attributes"
							v-on="events">
		...
		<div v-if="showCC" class="composer-fields">
			...
			<div class="composer-fields--custom">
				<NcSelect
					...
					:multiple="true"
					:placeholder="t('yoomail', 'Contact or email address …')"
					:aria-label-combobox="t('yoomail', 'Contact or email address …')"
					:clear-search-on-select="true"
		...
		<div class="composer-fields">
			...
			<input
				...
				class="subject"
				autocomplete="off"
				:placeholder="t('yoomail', 'Subject …')"
				@input="saveDraftDebounced">
		</div>
		<div v-if="noReply" class="warning noreply-warning">
			{{ t('yoomail', 'This message came from a noreply address so your reply will probably not be read.') }}
		</div>
		<div v-if="wantsSmimeEncrypt && missingSmimeCertificatesForRecipients.length" class="warning noreply-warning">
			{{
				t('yoomail', 'The following recipients do not have a S/MIME certificate: {recipients}.', {
		...
		<div v-if="encrypt && mailvelope.keysMissing.length" class="warning noreply-warning">
			{{
				t('yoomail', 'The following recipients do not have a PGP key: {recipients}.', {
		...
		<div class="composer-fields message-editor">
			...
			<TextEditor
				...
				name="body"
				class="message-body"
				:placeholder="t('yoomail', 'Write message …')"
		...
		<div class="composer-actions-right composer-actions">
			<div class="composer-actions--primary-actions">
				<p class="composer-actions-draft-status">
					<span v-if="savingDraft" class="draft-status">{{ t('yoomail', 'Saving draft …') }}</span>
					<span v-else-if="!canSaveDraft" class="draft-status">{{ t('yoomail', 'Error saving draft') }}</span>
					<span v-else-if="draftSaved" class="draft-status">{{ t('yoomail', 'Draft saved') }}</span>
				</p>
				<ButtonVue
					v-if="!savingDraft && !canSaveDraft"
					class="button"
					type="tertiary"
					:aria-label="t('yoomail', 'Save draft')"
					@click="saveDraft">
					<template #icon>
						<Download :size="20" :title="t('yoomail', 'Save draft')" />
					</template>
				</ButtonVue>
				<ButtonVue
					...
					class="button"
					type="tertiary"
					:aria-label="t('yoomail', 'Discard & close draft')"
					@click="$emit('discard-draft')">
					<template #icon>
						<Delete :size="20" :title="t('yoomail', 'Discard & close draft')" />
					</template>
				</ButtonVue>
			</div>
			<div class="composer-actions--secondary-actions">
				<ButtonVue
					...
					type="tertiary"
					:aria-label="t('yoomail', 'Enable formatting')"
					@click="setEditorModeHtml()">
					<template #icon>
						<IconFormat :size="20" :title="t('yoomail', 'Enable formatting')" />
					</template>
				</ButtonVue>
				<ButtonVue
					v-if="!encrypt && !editorPlainText"
					type="tertiary"
					:pressed="true"
					:aria-label="t('yoomail', 'Disable formatting')"
					@click="setEditorModeText()">
					<template #icon>
						<IconFormat :size="20" :title="t('yoomail', 'Disable formatting')" />
					</template>
				</ButtonVue>
				...
				<Actions :open.sync="isAddAttachmentsOpen">
					...
					<ActionButton :close-after-click="true" @click="onAddCloudAttachment">
						...
						{{
							t('yoomail', 'Add attachment from Files')
						}}
					</ActionButton>
					<ActionButton :close-after-click="true" :disabled="encrypt" @click="onAddCloudAttachmentLink">
						...
						{{
							t('yoomail', 'Add share link from Files')
						}}
					</ActionButton>
				</Actions>
					...
					<template v-if="!isMoreActionsOpen">
						...
						<ActionButton :close-after-click="true" @click="openTextBlockPicker">
							<template #icon>
								<NcIconSvgWrapper
									:size="20"
									:title="t('yoomail', 'Text blocks')"
									:svg="textBlockSvg" />
							</template>
							{{
								t('yoomail', 'Text blocks')
							}}
						</ActionButton>
						<ActionButton
							...
							<template #icon>
								<SendClock :size="20" :title="t('yoomail', 'Send later')" />
							</template>
							{{
								t('yoomail', 'Send later')
							}}
						</ActionButton>
						<ActionCheckbox
							...
							{{ t('yoomail', 'Request a read receipt') }}
						</ActionCheckbox>
						<ActionCheckbox
							...
							{{ t('yoomail', 'Sign message with S/MIME') }}
						</ActionCheckbox>
						<ActionCheckbox
							v-if="smimeCertificateForCurrentAlias"
							...
							{{ t('yoomail', 'Encrypt message with S/MIME') }}
						</ActionCheckbox>
						<ActionCheckbox
							...
							{{ t('yoomail', 'Encrypt message with Mailvelope') }}
						</ActionCheckbox>
					</template>
					<template v-if="isMoreActionsOpen">
						<ActionButton
						...
						<ActionRadio
							...
							{{ t('yoomail', 'Tomorrow morning') }} - {{ convertToLocalDate(dateTomorrowMorning) }}
						</ActionRadio>
						<ActionRadio
							v-model="sendAtVal"
							...
							:name="sendAtVal.toString()"
							class="send-action-radio send-action-radio--multiline">
							{{ t('yoomail', 'Tomorrow afternoon') }} - {{ convertToLocalDate(dateTomorrowAfternoon) }}
						</ActionRadio>
						<ActionRadio
							v-model="sendAtVal"
							...
							:name="sendAtVal.toString()"
							class="send-action-radio send-action-radio--multiline">
							{{ t('yoomail', 'Monday morning') }} - {{ convertToLocalDate(dateMondayMorning) }}
						</ActionRadio>
						<ActionRadio
							v-model="sendAtVal"
							:name="sendAtVal.toString()"
							...
							{{ t('yoomail', 'Custom date and time') }}
						</ActionRadio>
						<ActionInput
	...
	computed: {
		...
		submitButtonTitle() {
			if (this.wantsSmimeEncrypt) {
				if (this.sendAtVal) {
					return t('yoomail', 'Encrypt with S/MIME and send later') + ` ${this.convertToLocalDate(this.sendAtVal)}`
				}
				return t('yoomail', 'Encrypt with S/MIME and send')
			}

			if (this.mailvelope.available && this.encrypt) {
				if (this.sendAtVal) {
					return t('yoomail', 'Encrypt with Mailvelope and send later') + ` ${this.convertToLocalDate(this.sendAtVal)}`
				}
				return t('yoomail', 'Encrypt with Mailvelope and send')
			}

			if (this.sendAtVal) {
				return t('yoomail', 'Send later') + ` ${this.convertToLocalDate(this.sendAtVal)}`
			}
			return t('yoomail', 'Send')
		},
	...
	mounted() {
		...
		for (const id of this.forwardedMessages) {
			...
			if (!env) {
				...
				showError(t('yoomail', 'Message {id} could not be found', {
					id,
				}))
				continue
			}
	...
	methods: {
		...
		onAliasChange(alias) {
			...
			if (this.wantsSmimeSign || this.wantsSmimeEncrypt) {
				if (!this.smimeCertificateForAlias(alias)) {
					...
					showWarning(t('yoomail', 'Sign or Encrypt with S/MIME was selected, but we don\'t have a certificate for the selected alias. The message will not be signed or encrypted.'))
				}
			}
		...
		setEditorModeText() {
			OC.dialogs.confirmDestructive(
				t('yoomail', 'Any existing formatting (for example bold, italic, underline or inline images) will be removed.'),
				t('yoomail', 'Turn off formatting'),
				{
					type: OC.dialogs.YES_NO_BUTTONS,
					confirm: t('yoomail', 'Turn off and remove formatting'),
					confirmClasses: 'error',
					cancel: t('yoomail', 'Keep formatting'),
				},
				(decision) => {
					if (decision) {
