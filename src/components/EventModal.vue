...
<template>
	<Modal
		size="large"
		:name="t('yoomail', 'Create event')"
		@close="onClose">
		<div class="modal-content">
			<h2>{{ t('yoomail', 'Create event') }}</h2>
			<div class="eventTitle">
				<label for="eventTitle">{{ t('yoomail', 'Title') }}</label>
				<input
					id="eventTitle"
					v-model="eventTitle"
			...
			<div class="attendees-field">
				<label for="attendees">{{ t('yoomail', 'Attendees') }}</label>
				<span v-if="!organizerEmail" class="attendees-disabled-msg">
					{{ t('yoomail', 'You can only invite attendees if your account has an email address set') }}
				</span>
				<NcSelect
					v-else
					id="attendee"
			...
			<NcSelect
				...
				label="displayname"
				:aria-label-combobox="t('yoomail', 'Select calendar')"
				:options="calendars">
			...
			<label for="description">{{ t('yoomail', 'Description') }}</label>
			<textarea
				id="description"
				v-model="description"
				:disabled="generatingData"
...
export default {
	...
	data() {
		...
		return {
			...
			llmProcessingEnabled: loadState('yoomail', 'llm_summaries_available', false),
			attendeesList: [],
			attendeesOptions: [],
		}
	},
	...
	methods: {
		...
		async generateEventData() {
			...
			try {
				...
				this.description = description + '\n\n' + t('yoomail', 'This description was generated by AI.')
			} catch (error) {
		...
		async onSave() {
			...
			try {
				...
				showSuccess(t('yoomail', 'Event created'))
				this.onClose()
			} catch (error) {
				showError(t('yoomail', 'Could not create event'))
