...
<template>
	<ConfirmationModal
		...
		@confirm="deleteTag"
		@cancel="onClose">
		{{ t('yoomail', 'The tag will be deleted from all messages.') }}
	</ConfirmationModal>
</template>
	...
	methods: {
		...
		async deleteTag() {
			...
			try {
				...
				showSuccess(t('yoomail', 'Tag: {name} deleted', { name: this.tag.displayName }))
			} catch (error) {
				showInfo(t('yoomail', 'An error occurred, unable to delete the tag.'))
			} finally {
				this.deleting = false
				this.onClose()
			}
