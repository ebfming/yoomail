	...
	<div>
			...
			<li v-if="showForm">
				<form id="createAliasForm" @submit.prevent="createAlias">
					...
					<input
						v-model="newAlias"
						type="email"
						:placeholder="t('yoomail', 'Email address')"
						required>
				</form>
			</li>
		</ul>
		...
		<div v-if="!account.provisioningId" class="aliases-controls">
			<ButtonVue
				v-if="!showForm"
				type="primary"
				:aria-label="t('yoomail', 'Add alias')"
				@click="showForm = true">
				{{ t('yoomail', 'Add alias') }}
			</ButtonVue>
			...
			<ButtonVue
				...
				:aria-label="t('yoomail', 'Create alias')"
				:disabled="loading">
				<template #icon>
					<IconLoading v-if="loading" :size="20" />
					<IconCheck v-else :size="20" />
				</template>
				{{ t('yoomail', 'Create alias') }}
			</ButtonVue>
			<ButtonVue
				v-if="showForm"
				type="tertiary-no-background"
				class="button-text"
				:aria-label="t('yoomail', 'Cancel')"
				@click="resetCreate">
				{{ t("yoomail", "Cancel") }}
			</ButtonVue>
		</div>
