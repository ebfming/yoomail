OC.L10N.register(
    "yoomail",
    {
    "Work" : "Праца",
    "Personal" : "Асабістыя",
    "Mail" : "Пошта",
    "You are reaching your mailbox quota limit for {account_email}" : "Вы дасягаеце ліміту квоты вашай паштовай скрынкі для {account_email} ",
    "Drafts are saved in:" : "Чарнавікі захоўваюцца ў:",
    "Sent messages are saved in:" : "Адпраўленыя паведамленні захоўваюцца ў:",
    "Deleted messages are moved in:" : "Выдаленыя паведамленні перамяшчаюцца ў:",
    "Archived messages are moved in:" : "Архіваваныя паведамленні перамяшчаюцца ў:",
    "Snoozed messages are moved in:" : "Адкладзеныя паведамленні перамяшчаюцца ў:",
    "Sign in with Google" : "Увайсці з дапамогай Google",
    "Sign in with Microsoft" : "Увайсці з дапамогай Microsoft",
    "Save" : "Захаваць",
    "Loading account" : "Загрузка ўліковага запісу",
    "Account updated" : "Уліковы запіс абноўлены",
    "Name" : "Назва",
    "name@example.org" : "name@example.org",
    "Password" : "Пароль",
    "IMAP Settings" : "Налады IMAP",
    "IMAP Host" : "Хост IMAP",
    "IMAP Security" : "Бяспека IMAP",
    "None" : "Няма",
    "SSL/TLS" : "SSL/TLS",
    "STARTTLS" : "STARTTLS",
    "IMAP Port" : "Порт IMAP",
    "IMAP User" : "Карыстальнік IMAP",
    "IMAP Password" : "Пароль IMAP",
    "SMTP Settings" : "Налады SMTP",
    "SMTP Host" : "Хост SMTP",
    "SMTP Security" : "Бяспека SMTP",
    "SMTP Port" : "Порт SMTP",
    "SMTP User" : "Карыстальнік SMTP",
    "SMTP Password" : "Пароль SMTP",
    "Signature" : "Подпіс",
    "Default folders" : "Прадвызначаныя папкі",
    "Filters" : "Фільтры",
    "Quick actions" : "Хуткія дзеянні",
    "Email address" : "Адрас электроннай пошты",
    "Go back" : "Назад",
    "Cancel" : "Скасаваць",
    "General" : "Агульныя",
    "Appearance" : "Знешні выгляд",
    "Layout" : "Макет",
    "List" : "Спіс",
    "Sorting" : "Сартаванне",
    "Newest first" : "Спачатку новыя",
    "Oldest first" : "Спачатку старыя",
    "Ok" : "ОК",
    "Privacy" : "Прыватнасць",
    "Security" : "Бяспека",
    "S/MIME" : "S/MIME",
    "Manage certificates" : "Кіраванне сертыфікатамі",
    "Step 1" : "Крок 1",
    "Install the browser extension" : "Усталюйце пашырэнне для браўзера",
    "Step 2" : "Крок 2",
    "Enable for the current domain" : "Уключыце для бягучага дамена",
    "Archive" : "Архіў",
    "Delete" : "Выдаліць",
    "Search" : "Пошук",
    "Send" : "Адправіць",
    "Refresh" : "Абнавіць",
    "About" : "Аб праграме",
    "Send later" : "Адправіць пазней",
    "From" : "Ад",
    "Select account" : "Выберыце ўліковы запіс",
    "To" : "Каму",
    "Cc/Bcc" : "Cc/Bcc",
    "Contact or email address …" : "Кантакт або email …",
    "Cc" : "Cc",
    "Bcc" : "Bcc",
    "Subject" : "Тэма",
    "Subject …" : "Тэма …",
    "Write message …" : "Напішыце паведамленне …",
    "Saving draft …" : "Захаванне чарнавіка …",
    "Error saving draft" : "Памылка пры захаванні чарнавіка",
    "Draft saved" : "Чарнавік захаваны",
    "Save draft" : "Захаваць чарнавік",
    "Discard & close draft" : "Адхіліць і закрыць чарнавік",
    "Enable formatting" : "Уключыць фарматаванне",
    "Disable formatting" : "Выключыць фарматаванне",
    "Smart picker" : "Smart picker",
    "Send now" : "Адправіць зараз",
    "Tomorrow morning" : "Заўтра ранкам",
    "Tomorrow afternoon" : "Заўтра ў другой палове дня",
    "Enter a date" : "Увядзіце дату",
    "_{count} attachment_::_{count} attachments_" : ["{count} далучэнне","{count} далучэнні","{count} далучэнняў","{count} далучэнняў"],
    "Confirm" : "Пацвердзіць",
    "Rich text" : "Фарматаваны тэкст",
    "No messages in this folder" : "У гэтай папцы няма паведамленняў",
    "No messages" : "Няма паведамленняў",
    "Choose" : "Выбраць",
    "Later today – {timeLocale}" : "Пазней сёння – {timeLocale}",
    "Set reminder for later today" : "Задаць напамін на пазней сёння",
    "Tomorrow – {timeLocale}" : "Заўтра – {timeLocale}",
    "Set reminder for tomorrow" : "Задаць напамін на заўтра",
    "This weekend – {timeLocale}" : "У гэты ўік-энд – {timeLocale}",
    "Set reminder for this weekend" : "Задаць напамін на гэты ўік-энд",
    "Next week – {timeLocale}" : "На наступным тыдні – {timeLocale}",
    "Set reminder for next week" : "Задаць напамін на наступны тыдзень",
    "Encrypted message" : "Зашыфраванае паведамленне",
    "Favorite" : "Абранае",
    "Unread" : "Непрачытанае",
    "Read" : "Чытаць",
    "Edit tags" : "Рэдагаваць тэгі",
    "More actions" : "Больш дзеянняў",
    "Back" : "Назад",
    "Create task" : "Стварыць заданне",
    "Delete message" : "Выдаліць паведамленне",
    "_Forward {number} as attachment_::_Forward {number} as attachment_" : ["Пераслаць {number} як далучэнне","Пераслаць {number} як далучэнне","Пераслаць {number} як далучэнне","Пераслаць {number} як далучэнне"],
    "Mark as unread" : "Пазначыць як непрачытанае",
    "Mark as read" : "Пазначыць як прачытанае",
    "Title" : "Загаловак",
    "All day" : "Увесь дзень",
    "Attendees" : "Удзельнікі",
    "Description" : "Апісанне",
    "Create" : "Стварыць",
    "Select" : "Выбраць",
    "Comment" : "Каментарый",
    "Accept" : "Прыняць",
    "Decline" : "Адхіліць",
    "More options" : "Больш параметраў",
    "Remove" : "Выдаліць",
    "Last hour" : "Апошняя гадзіна",
    "Today" : "Сёння",
    "Yesterday" : "Учора",
    "Last week" : "Мінулы тыдзень",
    "Last month" : "Мінулы месяц",
    "Loading messages …" : "Загрузка паведамленняў …",
    "Choose target folder" : "Выберыце мэтавую папку",
    "No results" : "Няма вынікаў",
    "Favorites" : "Абранае",
    "Other" : "Іншае",
    "Direct link copied to clipboard" : "Прамая спасылка скапіявана ў буфер абмену",
    "Forward" : "Пераслаць",
    "Link copied" : "Спасылка скапіявана",
    "Move message" : "Перамясціць паведамленне",
    "Translate" : "Перакласці",
    "Forward message as attachment" : "Пераслаць паведамленне як далучэнне",
    "Unnamed" : "Без назвы",
    "Download Zip" : "Спампаваць Zip",
    "Quoted text" : "Цытаваны тэкст",
    "Move" : "Перамясціць",
    "Change password" : "Змяніць пароль",
    "Used quota: {quota}% ({limit})" : "Выкарыстана квота: {quota}% ({limit})",
    "Used quota: {quota}%" : "Выкарыстана квота: {quota}%",
    "Remove account" : "Выдаліць уліковы запіс",
    "Remove {email}" : "Выдаліць {email}",
    "Add folder" : "Дадаць папку",
    "Folder name" : "Назва папкі",
    "Saving" : "Захаванне",
    "Move up" : "Перамясціць уверх",
    "Move down" : "Перамясціць уніз",
    "Show all folders" : "Паказаць усе папкі",
    "_{total} message_::_{total} messages_" : ["{total} паведамленне","{total} паведамленні","{total} паведамленняў","{total} паведамленняў"],
    "Loading …" : "Загрузка …",
    "Clear folder" : "Ачысціць папку",
    "Delete folder" : "Выдаліць папку",
    "Delete folder {name}" : "Выдаліць папку {name}",
    "Add subfolder" : "Дадаць падпапку",
    "Rename" : "Перайменаваць",
    "Move folder" : "Перамясціць папку",
    "Clear cache" : "Ачысціць кэш",
    "Delete all messages" : "Выдаліць усе паведамленні",
    "Translate this message to {language}" : "Перакласці гэта паведамленне на {language}",
    "New message" : "Новае паведамленне",
    "Edit message" : "Рэдагаваць паведамленне",
    "Draft" : "Чарнавік",
    "Reply" : "Адказаць",
    "Message saved" : "Паведамленне захавана",
    "Failed to save message" : "Не ўдалося захаваць паведамленне",
    "Failed to save draft" : "Не ўдалося захаваць чарнавік",
    "attachment" : "далучэнне",
    "Message discarded" : "Паведамленне адхілена",
    "Could not discard message" : "Не ўдалося адхіліць паведамленне",
    "Retry" : "Паўтарыць спробу",
    "Message" : "Паведамленне",
    "New Contact" : "Новы кантакт",
    "Copy to clipboard" : "Капіяваць у буфер абмену",
    "Contact name …" : "Імя кантакту …",
    "Add" : "Дадаць",
    "Clear" : "Ачысціць",
    "Search in folder" : "Пошук у папцы",
    "Close" : "Закрыць",
    "Date" : "Дата",
    "Pick a start date" : "Выберыце дату пачатку",
    "Pick an end date" : "Выберыце дату заканчэння",
    "Tags" : "Тэгі",
    "Custom" : "Карыстальніцкі",
    "Signature …" : "Подпіс …",
    "Saving new tag name …" : "Захаванне новай назвы тэга …",
    "Delete tag" : "Выдаліць тэг",
    "Add tag" : "Дадаць тэг",
    "Saving tag …" : "Захаванне тэга …",
    "Task created" : "Заданне створана",
    "Could not create task" : "Не ўдалося стварыць заданне",
    "No calendars with task list support" : "Няма календароў з падтрымкай спісаў заднняў",
    "Not found" : "Не знойдзена",
    "From:" : "З:",
    "To:" : "Да:",
    "Go to latest message" : "Перайсці да апошняга паведамлення",
    "Newest message" : "Апошняе паведамленне",
    "Please select languages to translate to and from" : "Выберыце, з якой мовы на якую перакладаць",
    "The message could not be translated" : "Не ўдалося перакласці паведамленне ",
    "Translation copied to clipboard" : "Пераклад скапіяваны ў буфер абмену",
    "Translation could not be copied" : "Не ўдалося скапіяваць пераклад",
    "Translate message" : "Перакласці паведамленне",
    "Source language to translate from" : "Зыходная мова для перакладу",
    "Translate from" : "Перакласці з",
    "Target language to translate into" : "Мэтавая мова для перакладу",
    "Translate to" : "Перакласці на",
    "Translating" : "Пераклад",
    "Copy translated text" : "Скапіяваць перакладзены тэкст",
    "Untitled event" : "Падзея без назвы",
    "(organizer)" : "(арганізатар)",
    "Train" : "Цягнік",
    "Mark message as" : "Пазначыць паведамленне як",
    "Move into folder" : "Перамясціць у папку",
    "Stop" : "Спыніць",
    "Answered" : "Адказанае",
    "Deleted" : "Выдаленае",
    "Seen" : "Прачытанае",
    "Delete filter" : "Выдаліць фільтр",
    "New filter" : "Новы фільтр",
    "Filter saved" : "Фільтр захаваны",
    "Could not save filter" : "Не ўдалося захаваць фільтр",
    "Filter deleted" : "Фільтр выдалены",
    "Could not delete filter" : "Не ўдалося выдаліць фільтр",
    "Filter is active" : "Фільтр актыўны",
    "Filter is not active" : "Фільтр неактыўны",
    "Help" : "Даведка",
    "Enter subject" : "Увядзіце тэму",
    "Conditions" : "Умовы",
    "Add condition" : "Дадаць умову",
    "Actions" : "Дзеянні",
    "Add action" : "Дадаць дзеянне",
    "Priority" : "Прыярытэт",
    "Enable filter" : "Уключыць фільтр",
    "Tag" : "Тэг",
    "delete" : "выдаліць",
    "Edit" : "Рэдагаваць",
    "Reset" : "Скінуць",
    "Client ID" : "Ідэнтыфікатар кліента",
    "IMAP: {user} on {host}:{port} ({ssl} encryption)" : "IMAP: {user} на {host}:{port} (шыфраванне {ssl})",
    "SMTP: {user} on {host}:{port} ({ssl} encryption)" : "SMTP: {user} на {host}:{port} (шыфравенне {ssl})",
    "Sieve: {user} on {host}:{port} ({ssl} encryption)" : "Sieve: {user} на {host}:{port} (шыфраванне {ssl})",
    "Email address template" : "Шаблон адраса электроннай пошты",
    "IMAP" : "IMAP",
    "User" : "Карыстальнік",
    "Host" : "Хост",
    "Port" : "Порт",
    "SMTP" : "SMTP",
    "Sieve" : "Sieve",
    "S/MIME certificates" : "Сертыфікаты S/MIME",
    "Certificate name" : "Назва сертыфіката",
    "E-mail address" : "Адрас электроннай пошты",
    "Valid until" : "Дзейнічае да",
    "Delete certificate" : "Выдаліць сертыфікат",
    "PKCS #12 Certificate" : "Сертыфікат PKCS #12",
    "PEM Certificate" : "Сертыфікат PEM",
    "Certificate" : "Сертыфікат",
    "Submit" : "Адправіць",
    "Guest" : "Госць",
    "Group" : "Група",
    "Shared" : "Абагуленыя",
    "Edit {title}" : "Рэдагаваць {title}",
    "Delete {title}" : "Выдаліць {title}",
    "Insert" : "Уставіць",
    "All" : "Усе",
    "Drafts" : "Чарнавікі",
    "Priority inbox" : "Прыярытэтныя",
    "Inbox" : "Уваходныя",
    "Sent" : "Адпраўленыя",
    "Error while sharing file" : "Памылка пры абагульванні файла",
    "_%n new message \nfrom {from}_::_%n new messages \nfrom {from}_" : ["%n новае паведамленне \nад {from}","%n новыя паведамленні \nад {from}","%n новых паведамленняў \nад {from}","%n новых паведамленняў \nад {from}"],
    "Discard changes" : "Адхіліць змены",
    "Discard unsaved changes" : "Адхіліць незахаваныя змены",
    "Sending message…" : "Адпраўка паведамлення…",
    "Message sent" : "Паведамленне адпраўлена",
    "Could not send message" : "Не атрымалася адправіць паведамленне",
    "Message copied to \"Sent\" folder" : "Паведамленне скапіявана ў папку \"Адпраўленыя\"",
    "Could not copy message to \"Sent\" folder" : "Не ўдалося скапіяваць паведамленне ў папку \"Адпраўленыя\"",
    "Could not load {tag}{name}{endtag}" : "Не ўдалося загрузіць {tag}{name}{endtag}",
    "There was a problem loading {tag}{name}{endtag}" : "Узнікла праблема з загрузкай {tag}{name}{endtag}",
    "Could not load your message" : "Не ўдалося загрузіць ваша паведамленне",
    "Could not load the message" : "Не ўдалося загрузіць паведамленне",
    "Date:" : "Дата:",
    "Error loading message" : "Памылка пры загрузцы паведамлення"
},
"nplurals=4; plural=(n%10==1 && n%100!=11 ? 0 : n%10>=2 && n%10<=4 && (n%100<12 || n%100>14) ? 1 : n%10==0 || (n%10>=5 && n%10<=9) || (n%100>=11 && n%100<=14)? 2 : 3);");
